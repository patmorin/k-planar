...
\begin{lem}
Say $G_0$ is a 2-connected plane multigraph. Let $T$ be a spanning tree of $G_0$ rooted at some vertex $r$ on the boundary of the outerface of $G_0$. Let $k\in\{1,2,3\}$ and let $C = [A_1,\dots,A_k]$ be a cycle in $G_0$ with $r$ in the exterior of $C$, where $A_1, \dots, A_k$ are bipods in $G_0$. Let $G$ be the plane graph consisting of all the edges and vertices of $G_0$ in $C$ and the interior of $C$. Then $G$ has a partition $\PP$ into tripods with respect to $T$ such that:
\begin{compactitem}
\item $P_1,\dots,P_k\in\PP$,
\item the quotient $G / \PP$ is planar and has a tree-decomposition $\mathcal{T}$ of width at most 3 such that:
\begin{compactitem}
	\item some bag of $\mathcal{T}$ contains $P_1,\dots,P_k$, and 
	\item for every face $F$ of $G$ there is a bag $B$ of $\mathcal{T}$ such that $V(F)\subseteq \bigcup_{X\in B} X$.
\end{compactitem}
\end{compactitem}
\end{lem}

\begin{proof}
	\note{DW}{Can we always assume $k=3$ by allowing $A_i=\emptyset$? That would clean it up.}
	
	\note{DW}{Is $C$ allowed to be a 2-cycle?}
The proof is by induction on $|V(G)\setminus V(C)|$. If $V(G)\setminus V(C)=\emptyset$, then $G$ has just one face bounded by $C$ and the desired partition is $\PP=\{P_1,\ldots,P_k\}$.  The tree-decomposition of $G/\PP$ consists of a single bag that contains the $k\le 3$ vertices corresponding to $P_1,\ldots,P_k$.
...
Let $Y$ be the tripod consisting of $Q_1'\cup Q_2' \cup Q_3'$ plus the vertices and edges of $F$.

Say $F=v_1 S_1 v_2 S_2 v_3 S_3$, where $S_i$ is a facial path in $F$ clockwise from $v_i$ to $v_{i+1}$.

Let $L_i$ be the path $Q_i'S_iQ_{i+1}'$ for $i\in\{1,\dots,k\}$. So $L_i$ is a bipod. 

, where we use the convention that $Q_4=Q_1$ and $R_4=R_1$. 
...
Consider the plane graph $Z$ consisting of the edges and vertices of $F$, the edges and vertices of $C$, and the edges and vertices of $Q_1,Q_2,Q_3$. Then $Z$ is 2-connected, each face of $Z$ is bounded by a cycle, and the outerface of $Z$ is $C$. Let $Z_1,\dots,Z_m$ be the internal faces of $Z$. 

Consider $j\in\{1,\dots,m\}$. Let $G_j$ be the subgraph of $G$ consisting of all the edges and vertices of $G$ in $Z_j$ and the interior of $Z_j$. The outerface of $G_j$ is $Z_j$. Observe that for some $i\in\{1,\dots,k\}$, we have
$Z_j=[R_{i,j}^+,R_{i+1,j}^-,S_{i,j}]$, where 
$R_{i,j}^+$ is some subpath of $R_i^+$,
$R_{i+1,j}^-$ is some subpath of $R_{i+1}^-$, and
$S_{i,j}$ is some subpath of $S_i$.
Since $R_i^+$, $R_{i+1}^-$ and $S_i$ are bipods, 
$R_{i,j}^+$, $R_{i+1,j}^-$ and $S_{i,j}$ are bipods. 
By construction, $v_{i-1}$ is not in $G_j$. 
...
\begin{compactitem}
	\item $R_{i,j}^+, R_{i+1,j}^-, S_{i,j}\in\PP_j$,
	\item the quotient $G_j / \PP_j$ is planar and has a tree-decomposition $\mathcal{T}_j$ of width at most 3 such that: 
	\begin{compactitem}
		\item some bag $B_j$ of $\mathcal{T}_j$ contains $R_{i,j}^+, R_{i+1,j}^-, S_{i,j}$, and 
		\item for every face $F$ of $G_j$ there is a bag $B$ of $\mathcal{T}_j$ such that $V(F)\subseteq \bigcup_{X\in B} X$.
	\end{compactitem}
\end{compactitem}
\end{lem}
