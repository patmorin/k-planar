...
\note{PM}{Include in note to referee:  We choose to keep this section because
  $1$-planar graphs are of special interest to the graph drawing community.  For example, the annotated bibliography on $1$-planarity (\htmladdnormallink{https://arxiv.org/abs/1703.02261}{arXiv:1703.02261}) has 143 entries, and even this is now four years out of date.
...
Let $G$ be a $1$-plane graph.  
% Then $G$ has $O(V(G)$ edges since the edges of $G$ can be partitioned into two planar graphs by placing one edge from each crossing pair into a second set.
% \footnote{We may assume that $G$ has no parallel edges, since removing all but one copy from each set of parallel edges does not change the set of adjacent pairs of vertices in $G$, so any supergraph that contains}
We make $G$ into an \emph{edge-maximal} $1$-plane multigraph, by repeating the following operation:  If two vertices $v$ and $w$ of $G$ appear on a common face $F$ and there is no edge $vw\in E(G)$ contained in the boundary of $F$, then add $vw$, embedded in the face $F$.  This may introduce parallel edges into $G$, but no two such edges will appear on the boundary of a common face.  
% This increases the number of edges of $G$, by $O(|V(G)|)$ since these newly added edges do not cross any other edges of $G$.
...
If $W$ contains a dummy vertex $w_i$ then neither $w_{i-1}$ nor $w_{i+1}$ is a dummy vertex since if $w_{i-1}$ (respectively $w_{i+1}$) were a dummy vertex then the edge of $G$ that contains $w_{i-1}w_i$ (respectively $w_iw_{i+1}$) would be involved in at least two crossings.  Therefore each dummy vertex $w_i$ of $W$ is surrounded by two non-dummy vertices $w_{i-1},w_{i+1}\in V(G)$.  Since $w_i$ has degree $4$ in $G_0$, $w_{i-1}\neq w_{i+1}$.  Since $G$ is edge-maximal the edge $w_{i-1}w_i$ is an edge of $G$ on the boundary of $F$.  Therefore, if $F$ has a dummy vertex $w_i$ in its facial walk $W$, then $W=w_{i-1},w_i,w_{i+1}$ and $F$ is bounded by three edges of $G_0$, each of which is contained in a different edge of $G$.
...
By removing both spars from each kite of $G$ we obtain a plane multigraph $G'$ each of whose faces is bounded by three or four edges; see \cref{one_planar_example}(b).  Observe that, for any spar $xy\in E(G)\setminus E(G')$ that crosses $vw\in E(G')$, $G'$ contains the path $vxw$ (and $vyw$).  Therefore $\dist_{G'}(v,w)\le 2$.
...
\begin{lem}
	...
	\begin{compactenum}
		...
		\item Let $F$ be a cycle in $G'$ with $r$ in the exterior of $F$ and such that $V(F)$ can be partitioned into (possibly empty) sets $P_1,\ldots,P_3$ such that for each $i\in\{1,2,3\}$,
		\begin{compactenum}
...
% \note{DW}{Should the vertices already grabbed by $P_1,P_2,P_3$ be mentioned in the setup? The new paths $Q_1,Q_2,Q_3$ need to avoid the vertices already grabbed by $P_1,P_2,P_3$. Basically, I am concerned about what happens when distinct vertical paths in $T$ want to grab the same vertex.} \note{PM}{No, at this point we're only promising an $H$-partition of $N$, the part of $G$ contained in the uncrossed cycle $F$.}
	...
	\begin{compactenum}
		...
		\item for each $i\in\{1,2,3\}$, there exists some $x_i\in V(H)$ such that $P_i=S_{x_i}$; 
...
\begin{proof}
  ...
  The proof is by induction on the number of inner vertices of $N$, i.e., on $|V(N)\setminus V(F)|$.  If $N$ has no inner vertices, then we can simply take $\PP:=\{P_1,\ldots,P_k\}$ and verify that the preconditions of the lemma (the `setup') ensure that $\PP$ satisfies the requirements of the lemma.  Therefore, we may assume that $N$ has at least one inner vertex.
  ...
  Refer to \cref{boring_figure}.
	Let $M$ denote the subgraph of $G'$ containing the edges and vertices of $\overline{Y}$ and the edges and vertices of $F$ and let $F_1,\ldots,F_m,\tau$ be the inner faces of $M$.  By our choice of $\tau$, the graph $M$ is $2$-connected.  
  Furthermore, $V(F_i)$ can be partitioned into at most three sets $P_{i,1}$, $P_{i,2}$, and $P_{i,3}$ where $P_{i,1}\subset V(Y)$, $P_{i,2}\subseteq P_a$, $P_{i,3}\subseteq P_b$ for some $a,b\in\{1,2,3\}$, and $F_i[P_{i,j}]$ is a path, for each $j\in\{1,2,3\}$.  Therefore, we can apply induction using the cycle $F_i$ and the partition $P_{i,1},P_{i,2},P_{i,3}$ of $V(F_i)$.  The remainder of the proof is now identical to that in \citet{DJMMUW20}.
  
  % Furthermore, for each $i\in\{1,\ldots,m\}$, the vertices on the boundary $F_i$ are contained in $Y\cup P_a\cup P_b$ for some $a,b\in\{1,\ldots,k\}$. In particular if $k=3$ then there exists some $c\in\{1,2,3\}$ such that no vertex of $P_c$ is on the boundary of $F_i$.
  ...
  \begin{figure}
    \begin{center}
      ...
      % \end{tabular}
    \end{center}
    \caption{The `Sperner' triangle $v_1v_2v_3$ in $N''$ and the Sperner face $\tau$ in $N'$.}
    \label{boring_figure}
  \end{figure}
  ...
  The result of the induction on $N_i$ is a partition $\mathcal{P}_i$ with each of $P_{i,1}$, $P_{i,2}$ and $P_{i,3}$ in its own part. For $\mathcal{P}$ we use the partition
  \[
      \mathcal{P}:=\{Y, P_1,\ldots,P_k\} \cup \bigcup_{i=1}^m \{P\in\mathcal{P}_i: P\cap V(M)=\emptyset\} \enspace .
  \]
  All that remains is to verify that $\mathcal{P}$ satisfies Requirements~1--3.   
  \begin{compactenum}
    \item To see that $H:=G/\mathcal{P}$ is planar, observe that $G/\mathcal{P}=G'/\mathcal{P}$\todo{Explain why $G/\mathcal{P}=G'/\mathcal{P}$} and that each part $P\in\mathcal{P}$ induces a connected subgraph $G'[P]$ of $G'$.  Since $G'$ is planar and planarity is preserved under edge contractions, this implies that $G'/\mathcal{P}$ is planar.

    \item  The parts $P_1,P_2,P_3$ satisfy Requirement~2 by Assumption~4(b)ii,  we have already argued that the part $Y$ satisfies Requirement~2, and the inductive hypothesis ensures that each of the remaining parts satisfy Requirement~2.
    
    \item Requirement~3 is satisfied since $\mathcal{P}$ contains parts $P_1,\ldots,P_k$.
    
    \item Next we build a tree decomposition $(B_y:y\in V(T_0))$ of $H$ that satisifies Requirement~4.  
    % We show that $\tw(H)\le 3$ by giving a tree-decomposition of $H$ of width at most $3$. 
    ...
    The tree $T_0$ has root node $y_0$ with $B_{y_0}:=\{x_0,\ldots,x_k\}$.    
    For each $i\in\{1,\ldots,m\}$, let $N_i$ denote the subgraph of $N$ contained in $F_i$.  The inductive hypothesis ensures that $H_i:=N_i/\mathcal{P}_i$ has a tree decomposition $(B_z:z\in V(T_i))$ whose largest bag has size at most $4$ whose root bag $B_{z_i}$ contains the vertices $a_i,b_i,c_i\in V(H_i)$ obtained by contracting $P_{i,1}$, $P_{i,2}$, and $P_{i,3}$, respectively.   Recall that $P_{i,1}:=V(N_i)\cap Y$ and $P_{i,2}:=V(N_i)\cap P_b$ and $P_{i,3}:=V(N_i)\cap P_c$ for some $b,c\in\{1,\ldots,k\}$.  For each bag $B_z$, $z\in V(T_i)$, replace each occurrence of $a_i$ by $x_0$, replace each occurrence of $b_i$ by $x_b$ and replace each occurrence of $c_i$ by $x_c$.  Note that these replacements do not increase the size of any bag. Now add an edge that joins $y_0$ to $x_i$.
  \end{compactenum}
...
\begin{compactenum}[(i)]
	...
	\item for every facial cycle $F$ of $G$ there is a bag $B$ of $\mathcal{T}$ such that $V(F)\subseteq \bigcup_{X\in B} X$.
\end{compactenum}
\note{DW}{We need to replace (ii) by ``for every facial cycle $F$ of $G$ there is a bag $B$ of $\mathcal{T}$ such that every part of $\PP$ that intersects $F$ is in $B$''. Curiously, this implies (i) (since $C$ is a facial cycle of $G$).}
\end{compactitem}
\end{lem}
...
First suppose that $C$ is a 2-cycle. Thus $k\leq 2$. Let $e$ be one of the edges in $C$. Let $F$ be the internal facial cycle of $G$ that includes $e$. Since each facial cycle of $G_0$ has length at least 3, $F$ consists of a non-empty path $P$ plus the edge $e$. Let $C'$ be obtained by replacing $e$ in $C$ by $P$. Let $G':=G\langle{C'}\rangle$. Thus $G'$ has one fewer face than $G$. Note that $C'=[P_1,\dots,P_k,P]$ (\note{DW}{or it can be written that way?}).  By induction, $G'$ has a partition $\PP$ into tripods with respect to $T$ such that:
\begin{compactitem}
	\item $P_1,\dots,P_k,P\in\PP$,
	\item $G / \PP$ has a tree-decomposition $\mathcal{T}$ of width at most 3 such that:
	\begin{compactenum}[(i)]
	\item some bag of $\mathcal{T}$ contains $P_1,\dots,P_k,P$, and 
	\item for every facial cycle $F'$ of $G\blah{C'}$ there is a bag $B$ of $\mathcal{T}$ such that $V(F)\subseteq \bigcup_{X\in B} X$.
	\end{compactenum}
\end{compactitem}
The bag of $\mathcal{T}$ that contains $P_1,\dots,P_k,P$ satisfies property (ii) for $F$. 
Thus, $\PP$ and $\TT$ satisfy the desired properties for $G$. Now assume that $|V(C)|\geq 3$. 
...
\begin{compactitem}
	\item $R_{i,j}^+, R_{i+1,j}^-, L_{i,j}\in\PP_j$, 
	\item the quotient $G_j / \PP_j$ has a tree-decomposition $\mathcal{T}_j=\{B_x:x\in V(D_j)\}$ of width at most 3 such that: 
	\begin{compactitem}
		\item some bag $B_{u_j}$ of $\mathcal{T}_j$ contains $R_{i,j}^+, R_{i+1,j}^-, L_{i,j}$, and 
		\item for every facial cycle $F'$ of $G_j$ there is a bag $B$ of $\mathcal{T}_j$ such that $V(F')\subseteq \bigcup_{X\in B} X$.
	\end{compactitem}
\end{compactitem}
...
Every facial cycle $F_0$ of $G$ is a face of some $G_j$ or is $F$. In the latter case, $V(F)\subseteq Y \subseteq \bigcup_{X\in B_u} X$. In the first case, there is a bag $B_x$ of $\mathcal{T}_j$ such that $V(F)\subseteq \bigcup_{X\in B_x} X$. This property is maintained by the above replacements since 
...
Let $G$ be a 2-connected plane multigraph, in which every facial cycle has length in $\{3,\dots,d\}$. Let $C$ be the outer-cycle of $G$. Let $G^+$ be obtained from $G$ by adding a new vertex $r$ adjacent to every vertex on the outerface of $G$. So $G^+$ is a 2-connected plane multigraph and $G=G^+\blah{C}$. Let $T$ be a BFS spanning tree of $G^+$ rooted at $r$. Let $\mathcal{L}$ be the corresponding BFS layering. By \cref{NewLemma} \note{DW}{with improved (ii)}, $G$ has a partition $\PP$ into tripods with respect to $T$ such that $G / \PP$ has a tree-decomposition $\mathcal{T}$ of width at most 3 such that for every facial cycle $F$ of $G$ there is a bag $B$ of $\mathcal{T}$ such that every part of $\PP$ that intersects $F$ is in $B$. Since $G$ is a spanning subgraph of $\widehat{G}$, we may consider $\PP$ to be a partition of $\widehat{G}$, and $G/\PP \subseteq \widehat{G}/\PP$. Consider an edge $XY$ of $\widehat{G}/\PP$. By the definition of $\widehat{G}$, there is a facial cycle $F$ of $G$ such that both $X$ and $Y$ intersect $F$. By assumption, there is a bag $B$ of $\mathcal{T}$ such that every part of $\PP$ that intersects $F$ is in $B$. In particular, $X,Y\in B$. Hence $\TT$ is also a tree-decomposition of $\widehat{G}$. Each tripod in $\PP$ has at most $d$ vertices in each layer in $\mathcal{L}$. For each edge $vw$ of $\widehat{G}$, if $v\in L_i$ and $w\in L_j$ then $|i-j|\leq\floor{\frac{d}{2}}$. Grouping blocks of $\floor{\frac{d}{2}}$ consecutive layers gives a layering of $\widehat{H}$ with layered width $\floor{\frac{d}{2}}d$ with respect to $\PP$. By \cref{PartitionProduct}, $\widehat{G}$ is contained in $H \boxtimes P \boxtimes K_{ d  \floor{d/2} }$ where $H:=\widehat{G}/\PP$, which has treewidth 3.
\end{proof}
