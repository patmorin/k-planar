...
The main results of this paper generalise the following recent theorem of \citet{dujmovic.joret.ea:planar} to this setting, and are the first results of this type for non-proper-minor-closed graph classes.
...
Loosely speaking, \cref{PlanarBasic} says that every planar graph is contained in the product of a tree-like graph and a path. This enables combinatorial results for graphs of bounded 
...
\end{wrapfigure}
treewidth to be generalised for planar graphs (with different constants). \cref{PlanarBasic} has been the key tool in solving several well-known open problems. In particular, \citet{dujmovic.joret.ea:planar} proved that planar graphs have bounded queue-number (resolving a conjecture of \citet{HLR92} from 1992), and \citet{dujmovic.esperet.ea:planar}  proved that planar graphs have bounded non-repetitive chromatic number (resolving a conjecture of \citet{AGHR-RSA02} from 2002). 
...
The relationship between $k$-planar graphs and $(k+1,2)$-shortcut systems of planar graphs shows that \cref{modern-major-general} has immediate applications to $k$-planar graphs.  The following result shows that $k$-planar graphs admit bounded layered partitions. With \cref{PartitionProduct,MakeWidth1}, this proves \cref{kPlanarBasic}. % with $c_k\leq(4+o(1))k^5$.
...
For the remainder of this section, $G$ is a graph and $(\mathcal{L}:=\langle V_1,\ldots,V_h\rangle,\mathcal{Y}:=(Y_x: x\in V(H)))$ is a layered partition of $G$ of layered width $\ell$ where the treewidth of the quotient graph $H=G/\mathcal{Y}$ is at most $t$.  In addition, $\mathcal{T}:=(B_x:x\in V(T))$ is a tree-decomposition of $H$ obtained by applying \lemref{nice-decomposition} to $H$.  The following lemma shows how to interpret the pair $(\mathcal{Y},\mathcal{T})$ as a hierarchical decomposition of $G$; refer to \cref{fig:generalized-tripod}.
...
\note{W}{I think \clmref{generalized-tripod} should be presented as a stand-alone lemma. Then we repeat the statement of \cref{modern-major-general} followed by $\\begin \{proof\} .... \\end\{proof\}$.}
