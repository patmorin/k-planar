...
\begin{proof}
  ...
  Let $w$ be the first vertex of $F$ encountered on the path in $T$ from $v$ to the root of $T$.  (This implies that $w=v$ when $v$ is a vertex of $F$.)  Since $w\in V(F)$, $w\in P_i$ for some $i\in\{1,2,3\}$ and we define $\alpha(v):=i$.

  If $k<3$ then let $\tau:=v_1v_2v_3$ be any inner face of $N'$.  \note{PM}{Not quite that easy. We want $M$ to be $2$-connected so have to make sure $\overline{Y}$ attaches to at least two different vertices of $F$.}
  Otherwise, $k=3$, in which case we apply Sperner's Lemma to obtain an inner face $\tau:=v_1v_2v_3$ such that $\alpha(v_i)=i$ for each $i\in\{1,2,3\}$.
  ...
  The graph $M^+$ has some number of inner faces, all contained in the interior of $F$. Some of the inner faces of $M^+$ are Type~(ii) faces. Let $F_1,\ldots,F_m$ be the Type~(i) inner faces of $M^+$.
	...
	Observe that each of the faces $F_1,\ldots,F_m$ is contained in a single inner face of $M$.  Therefore, $V(F_i)$ can be partitioned into at most three sets $P_1'$, $P_2'$, and $P_3'$ where $P_1'\subset V(Y^+)$, $P_2'\subseteq P_a$, $P_3'\subseteq P_b$ for some $a,b\in\{1,2,3\}$, and $F_i[P_j']$ is a path, for each $j\in\{1,2,3\}$.\note{PM}{This latter requirement requires that $M^+$ be $2$-connected, we should mention this.}
  Therefore, we can apply induction using the cycle $F_i$ and the partition $P_1',P_2',P_3'$ of $V(F_i)$ to obtain the desired $H$-partition $\PP_i$ and tree decomposition of subgraph $N_i$ of $G$ contained in $F_i$.
