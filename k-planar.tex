...
If a graph class admits bounded layered partitions, then it also has bounded layered treewidth. In particular, suppose that an $n$-vertex graph $G$ has an $H$-partition with layered width at most $\ell$ such that $H$ has treewidth at most $t$. \citet{dujmovic.joret.ea:planar} proved that $G$ has layered treewidth at most $(t+1)\ell$. 
...
Finally, we show that these results cannot be generalized to $k$-nearest-neighbour graphs of points in $\R^3$, even for $k=4$.
\begin{prop}\label{3d-4nn}
  The class of $4$-nearest-neighbour graphs of point sets in $\R^3$ does not admit bounded layered partitions.
\end{prop}
  
\begin{proof}
  We actually prove something stronger, that 4-nearest-neighbour graphs of point sets in $\R^3$ do not have bounded layered treewidth. As discussed in \cref{PriorWork}, if $G$ is a graph with an $H$-partition of layered width $\ell$ in which $\tw(H)\le t$, then the layered treewidth of $G$ is at most $(t+1)\ell$.  Next we give an examples of arbitrarily large $n$-vertex $4$-nearest-neighbour graphs in $\R^3$ having layered treewidth $\Omega(\sqrt{n}/\log n)$, thus establishing \cref{3d-4nn}.

  Let $r:=2^h$ for any integer $h>0$. For $i\in\{0,\ldots,h\}$, let $q_i:=\sum_{j=1}^i2^{i-2}$, let
  \[
     Z_i:=\left\{ q_i+x2^i, q_i+y2^i, 2^i)   : x,y\in\{0,r/2^{i}-1\} \right\}
  \]
  and define the quadtree-like point set $P = \bigcup_{i=0}^{h} Z_i$.  Let $G_P$ denote the $4$-nearest-neighbour graph of $P$. For any $p\in Z_0$, the distance to any point $q\in P\setminus Z_0$ is at least 
  \[ \sqrt{(1/2)^2 + (1/2)^2 + (2^1-2^0)} > 1 \enspace .\]
  Therefore $G_P[Z_0]$ contains an $r\times r$ grid. Therefore $G_P$ has treewidth at least $r-1$ and any tree decomposition $\mathcal{T}=(B_x:x\in V(T))$ of $G_P$ contains a bag $B_x$ of size at least $r$.

  We can readily verify that $G_P$ contains a complete $4$-ary spanning tree $T_4$ of height $h$.  Indeed, $T_4$ is rooted at the unique point in $Z_h$ and, for each $i\in\{0,\ldots,h\}$, the points in $Z_i$ have $T_4$-depth $h-i$.  Therefore $G_P$ has diameter at most $2h$. Therefore, any layering of $G_P$ has at most $2h$ non-empty layers.  Therefore, in any layering $\mathcal{L}:=\langle V_1,\ldots,V_{2h}\rangle$ of $G_P$, there is a layer $L_i\in\mathcal{L}$ such that $|L_i\cap B_x| > r/(2h) = r/(2\log_2 r)$.  Therefore the layered treewidth of $G_P$ is least $r/(2\log_2 r)$. Letting $n:=|V(G)| = \sum_{i=0}^h r^2/4^i < 2r^2$ we see that $G_P$ has layered treewidth $\Omega(\sqrt{n}/\log n)$.
\end{proof}
