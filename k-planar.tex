...
\note{PM}{I would also like to change $G'$ to $G_0$ and $N'$ to $N_0$.  One issue is holding me back, though: We recursive on cycles $F_1,\ldots,F_m$ that define subgraphs $N_1,\ldots,N_m$ of $G$ and subgraphs $N_1',\ldots,N_m'$ of $G'$.  Will these latter subgraphs becomes $N_{0,1},\ldots,N_{0,m}$?}
...
\cref{MapGraphProduct} is an immediate consequence of \cref{d_framed_product_stucture} and the next lemma. Similar results to \cref{MewMapGraphLemma} appear in earlier works \cite{CGP06,BDGGMR,Brandenburg19,Brandenburg20} but we state the precise lemma and provide a proof here for the sake of completeness.
...
\cref{MapGraphProduct,qn,p-centered,non-repetitive} imply the following result.

\begin{thm}
\label{PlaneMapApps}
Every $d$-map graph $G$:
	\begin{compactitem}
		\item has queue-number $\qn(G) \leq 24 (d+3\floor{d/2}-3) \,  - \ceil{\tfrac{3}{2} (d+3\floor{d/2}-3)}$.
		\item has $p$-centered chromatic number $\chi_p(G) \leq (d+3\floor{d/2}-3) (p+1)\,  \binom{p+3}{3}$,
		\item has non-repetitive chromatic number $ \pi(G) \leq 256(d+3\floor{d/2}-3) $.
	\end{compactitem}
\end{thm}
