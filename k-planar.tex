...
\begin{proof}
  ...
  \cref{i-ancestor} ensures that, for every edge $ax'\in E(J)$, both $a$ and $x$ are contained in $C_{x'}$\todo{Do we mean $C_x$ here?}.  The connectivity of $T[a]:=T[\{x\in V(T):a\in C_x\}]$ follows from the fact that, for every node $x'\in V(T[a])$, every node $x$ on the path in $T$ from $a$ to  $x'$ is also a node of $T[a]$.  Therefore $(C_x:x\in V(T))$ is indeed a tree-decomposition of $J$.
