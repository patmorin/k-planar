...
We note that \lemref{nearest-neighbour} is tight up to the leading constant:  Every $n$-vertex nearest neighbour graph has at least $kn/2$ at at most $kn$ edges.  For $k\ge 8$, the Crossing Lemma \cite{X} implies that the total number of crossings is therefore $\Omega(k^3n)$ so that the average number of crossings per edge is at $\Omega(k^2)$.
