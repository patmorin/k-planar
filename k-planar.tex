...
In \secref{g-k-planar} we prove \thmref{g-k-planar}. 
...
\begin{proof}[Proof of \thmref{k-planar}]
  ...
  By \clmref{bag-size}, $\mathcal{S}$ produces a graph $H=G/\mathcal{S}$ of treewidth at most $\treewidth$.  By \clmref{width-of-g-plus}, for each $x\in V(H)$ and each integer $i$, $|B_x\cap V_i|\le 24k+12$, so $|B_x\cap V_i'|\le (k+1)(24k + 12) = 24k^2+36k+12$. 
\end{proof}
