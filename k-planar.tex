...
We now generalize \thmref{k-planar} to a more abstract setting.  For a rooted tree $T$ and any node $x\in V(T)$, we define $T_x := T[\{y\in V(T):\mbox{$x$ is a $T$-ancestor of $y$}\}]$ to be the maximal subtree of $T$ rooted at $x$.
...
\begin{proof}[Proof of \clmref{generalized-tripod}]
  % Apply \lemref{nice-decomposition} to $H$ to obtain a tree-decomposition $(B_x: x\in V(T))$ of $H$ satisfying the conditions of \lemref{nice-decomposition}.
  % 
  % For each $v\in V(G)$, let $y(v)$ denote the unique node $y\in V(H)$ such that $v\in Y_y$.  Observe that, by \tref{subtree-root}, $y(v)$ is the root of $T[y(v)]=T[\{x\in V(T):y(v)\in B_x\}]$.  By definition, $\mathcal{Y}:=(Y_x:x\in V(T))$ is a partition of $V(G)$ and, for any $i\in\{1,\ldots,h\}$ $|Y_x\cap V_i|\le \ell$, so $\mathcal{Y}$ has layered width at most $\ell$ with respect to $\mathcal{L}$.  This establishes \yref{thickness}. 

  Property~\yref{thickness} follows immediately from the fact that $V(T)=V(H)$ and $\mathcal{Y}$ is a partition.  Property \yref{separator} is immediate from the definitions of $F_x$ and $N_x$.  In particular, $(N_x,V(G)\setminus V_x)$ is a separation of $G$ with $F_x=N_x\cap(V(G)\setminus V_x)$. 
