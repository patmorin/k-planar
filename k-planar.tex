...
\begin{proof}
  ...
  Refer to \figref{A}(b)
  If $B_x$ contains some $K$-ancestor $a_\delta$, $\delta\in\{0,\ldots,d-1\}$, of $x$, it is because there is an edge $vw\in E(G)$ with $w\in S_{a_\delta}$ and $v\in S_{x'}$ for some $K$-descendant of $x$.  The edge $vw$ corresponds to a walk $W_{vw}$ in $G^+$. The walk $W_{vw}$ is contained in $N_x-F_x$, otherwise $x\not\in A_{vw}$ so $x'\not\in A_{vw}$, so $x'\not\in A_v$, and $a(v)\neq x'$, contradicting the fact that $v\in S_{x'}$.
  ...
  \begin{compactenum}[(i)]
    ...
    \item $\ell\le k+1$.
  \end{compactenum}
  We call any walk with properties (i)--(iv) a \emph{$\delta$-walk}. Therefore, to upper-bound $|B_x|$, it suffices to upper-bound the number of $\delta\in\{0,\ldots,d-1\}$ such that there exists a $\delta$-walk.
  
  In order to avoid excessive levels of subscripts the remainder of this proof will slightly abuse notation in the following way: For $i\in\{0,\ldots,d\}$, we use $F_i$, $N_i$, and $Y_i$, as shorthands for $F_{a_i}$, $Y_{a_i}$, respectively.
  
  To begin, let $d_0=d$, and $d_1=\min\{i: V(F_d\cap Y_i)\neq\emptyset \}$.  Observe that $F_{d}=F_{d_0}$ is contained in the interior of $F_{d_1}$, i.e., $N_{d_0}\subseteq N_{d_1}-F_{d_1}$.   Refer to \figref{d0d1}.  We claim that 
  \[  
     |\{\delta\in\{d_1,\ldots,d_0-1\}:  \mbox{there exists a $\delta$-walk} \}| \le \binom{k+1}{2} \enspace .
  \]
  \begin{figure}
    \begin{center}
      \includegraphics{figs/d0d1-5}
    \end{center}
    \caption{$F_{d_1},\ldots,F_{d_0}$ and the resulting graph $I$.}
    \figlabel{d0d1}
  \end{figure}
  To prove this claim, consider the graph $I$ with vertex set $V(I)=\{d_1,\ldots,d_0\}$ and $ij\in E(I)$ if and only $i=j$ or $|F_i\cap Y_j|\neq\emptyset$.  Any $\delta$-walk $v_0,\ldots,v_\ell$ with $\delta\in V(I)$ corresponds to a walk $\rho_0,\ldots,\rho_{\ell}$ in $I$, where $\rho_i=\max\{j\in\{0,\ldots,d\} : v_i\in N_{j}-F_j\}$.  Because of Properties~(i)--(iv) of $\delta$-walks, any walk $\rho_0,\ldots,\rho_\ell$ obtained this way has (i)~$\rho_0=d_0$, (ii--iii)~$\delta=\rho_\ell < \min\{\rho_0,\ldots,\rho_{\ell-1}\}$, and (iv)~$\ell \le k+1$.
  
  The graph $I$ has the following properties:
  \begin{enumerate}
    \item For each $i\in V(i)$, $id_1\in E(I)$.
    \item For each $i\in V(I)$, $i$ has at most 2 neighbours in $\{d_1+1,\ldots,i-1\}$. If $i$ has 2 such neighbours, then one of them is $i-1$.

    \item For each $ij\in E(I)$ with $j<i-1$, and each $i'\in\{j+1,\ldots,i+1\}$, the neighbours of $i'$ in $\{d_1+1,\ldots,i'-1\}$ are contained in $\{i'-1,j\}$.
  \end{enumerate}
  These two properties imply that $I$ has a very simple structure.   We call $j\in\{d_1,\ldots,d_0\}$ a \emph{left endpoint} if there exists an edge $ij\in E(I)$ with $j < i - 1$.  Let $j_1>j_2>\cdots> j_p$ denote the sorted sequence of left endpoints and, for convenience, let $j_{0}=d$.  Then 
  \[
      E(I) \subseteq \{ (i,i+1): i\in\{d_1,\ldots,d_0-1\} \cup 
         \bigcup_{t=1}^p \{ ij_t : i\in\{ j_t,\ldots,j_{t-1} \}\} \enspace . 
  \]
  (See the lower half of \figref{d0d1}.)  Observe that shortest path from in $i$ from $d_0$ to $j_t$ has length at least $t$.  Let $X_t=\{j_t,\ldots,j_{t+1}\}$ and observe that the shortest path from $d_0$ to some value $i\in X_t$ that does not contain any $i' <i$ goes through $j_{t}$.  Therefore, $X_t$ contains at most $k+2-t$ values of $\delta$ such that there exists a $\delta$-walk.  Therefore, 
  \[  
     |\{\delta\in\{d_1,\ldots,d_0-1\}:  \mbox{there exists a $\delta$-walk} \}| \le \sum_{t=0}^{k+1} (k+2-t) = \sum_{t=1}^{k+2} t = \binom{k+2}{2} \enspace .
  \]
  
  % \begin{figure}
  %   \begin{center}
  %     \includegraphics{figs/i-graph}
  %   \end{center}
  %   \caption{The graph $I$ used in the proof of \clmref{bag-size}}
  %   \figlabel{i-graph}
  % \end{figure}
    
  More generally, for $s>1$, let $d_s=\min\{ i: V(F_{d_s}\cap Y_i)\neq\emptyset \}$. Consider some $\delta$-walk $v_0,\ldots,v_\ell$ for $\delta\in \{ d_{s},\ldots,d_{s-1}-1 \}$, for some $s>1$.  Since $v_0\in V(N_{d_0}-F_{d_0})$ and $v_\ell\in F_{\delta+1}-F_\delta$, there exists some maximum value $i$ such that $v_i\in N_{d_{s-1}}-F_{d_{s-1}}$.  Furthermore, $i\ge s-2$ because $v_1,\ldots,v_i$ contains a vertex from each of $F_{d_0},F_{d_1},\ldots,F_{d_{s-2}}$.  But this means that we can treat $v_i,\ldots,v_{\ell}$ as a $\delta$-walk of length at most $\ell-s+2 \le k-s+3$ that begins in $N_{d_{s-1}}-F_{d_{s-1}}$.  By the same argument used above, 
  \[  |\{\delta\in\{d_s,\ldots,d_{s-1}-1\}:  \mbox{there exists a $\delta$-walk} \}| \le \binom{k-s+3}{2} \enspace .\]
  
  We conclude that the maximum number of $\delta\in\{0,\ldots,d-1\}$ such that there exists a $\delta$-walk is at most
  \[
      \sum_{s=1}^{k+2} \binom{k-s+3}{2} = O(k^3)
  \]
  \note{PM}{Gotta run!}
  ...
  % 
  % 
  % Next observe that, if some $\delta$-walk
  % 
  % 
  % 
  % 
  ...
  % \[  
  %     d_{v'}=\max\{i\in\{0,\ldots,d\} : v'\in V(N_{a_i}-F_{a_i})
  % \]
  % (Intuitively, $d_{v'}$ is the $K$-depth of the lower endpoint of $K[P_{v'}]$ or the $K$-depth ($d$) of $x$, whichever is shallower.)
  % 
  % Consider some $\delta$-walk $v_0,\ldots,v_{\ell}$ and, for each $j\in\{0,\ldots,\ell\}$, let $d_j=d_{v_j}$.  Then the sequence $d_0,\ldots,d_{\ell}$ has the following properties, corresponding to properties (i)--(iii) above:
  % \begin{compactenum}[(i)]
  %   \item $d_0=d$;
  %   \item $d_0,\ldots,d_{\ell-1} > \delta$
  %   \item $d_\ell = \delta$;
  %   \item $\ell\le k+1$.
  % \end{compactenum}
  % 
  % Consider an auxilliary graph $I$ with vertex set $V(I)=\{0,\ldots,d\}$ and edge $ij\in E(I)$ if and only if $V(F_{a_i}\cap Y_j))\neq\emptyset$. Observe that $d_0,\ldots,d_\ell$ is a walk in $I$.
  % 
  % We are particularly interested in the \emph{up neighbours} of $i$, defined as
  % $U(i) = \{q : iq\in E(I),\, q< i-1\}$.  
  % 
  % Claim: For each $j\in\{i_1-2,\ldots,d\}$,  $U(j) = U(d)=\{i_1,i_2\}$.
  % 
  % Claim: $U(i_1)= \{i_2, i_3\}$ for some $i_3< i_1-1$.
  % 
  % Claim: $U(i_1+1)= \{i_1, i_2, i_3\}$.
  % 
  %  U(i)\cup\{i-2\}\setminus\{i-1\}$. 
  % 
  % Let $i_0=d$, $i_1=\min U(d)$, and $i_2=U(d)$.  
  % 
  % 
  % 
  % By \clmref{strong-ancestor-boundary}, $I(j)=U(d)=\{i_1,i_2\}$ for all $j\in\{i_1-1,\ldots,d\}$.  This immediately implies that there can be no $\delta$-walk for any $\delta\in \{ i\in\Z : k+1 < i < i_1 \}$. Indeed, such a walk either has to contain $i_1$ or $i_2$ or it has to contain $d,d-1,\ldots,d-k-2$.  In the former case, the walk violates (ii) or (iii) since it contains values smaller than $\delta$.  In the latter case, the walk violates (iv) since it has length $k+2>k+1$.
  % 
  % Next, consider $\max U(i_1)$ and let $i_3=\max U(i_1)$.  By \propref{strong-ancestor-boundary} 
  % 
  % 
  % 
  %  since $\delta > i_1 > i_2$ 
  % 
  % Claim: For all $j\in\{\min(U(i)),\ldots,i\}$, $U(j) = U(i)$. 
  % 
  % Proof: 
  % The proof is by induction on $i-j$.  The base case $i=j$ is immediate.
  % 
  % Now consider some $j\in\{\min(U(i)), i-1\}$.  By the inductive hypothesis $U(j+1)=U(i)$ so $F_{j+1}$ contains vertices of $Y_q$ for each $q\in U(i)$. 
  % These vertices are not in $Y_j$ since, by the definition of $U(j+1)$, $q < (j+1)-1 = j$ for each $q\in U(j+1)$.  
  % 
  % 
  %  Recall that $F_{j+1}$ is a wedge in the Mercedes graph $M_j$. This wedge is determined by the closed tripod $\overline{Y}_j$
  % 
  % First the base case $j=i$:
  % 
  % Observe that $F_i$ is one wedge of the Mercedes graph $M_i$, so $F_i \subset F_{i-1}\cup \overline{Y}_{i-1}$.  Therefore $V(F_i)$ can be partitioned into $A=V(F_i \cap F_{i-1})$ and $B=V(F_i\cap Y_{i-1})$.  The set $B$ consists entirely of vertices in $N_{i-1}-F_{i-1}-(N_i-F_i)$.  The set $A$ consists of vertices in $V(F_{i-1})$.  By \propref{ancestor-boundary}, 
  % 
  % First observe that it is sufficient to prove this for $j=i-1$ since, in this case, we can apply
  % Consider the vertices of $F_{i}$.  Each of these vertices belongs $Y_{q}$ for some $q\in U(i)$.
  % 
  % A critical property of these up edges is that, for any $i' > i$, $U(i') \subseteq \{i'+1,\ldots,i\} \cup U(i)$.  
  % 
  % 
  % , there are at most 3 edges $ij\in E(I)$ with $j<i$.  Furthermore, for each $i'< i$, 
  % 
  % 
  % 
  % The graph $I$ has some additional structure, which we now describe.  
  % First, consider the at most three neighbours $i_0>i_1>i_2$ of $d$ in $I$.
  % In the walk $d_0,\ldots,d_{\ell}$, the first value $d_i\neq d$ is in $\{i_0,i_1,i_2\}$.  Furthermore, 
  % 
  % 
  % 
  % 
  % First observe that, for each $i\in\{1,\ldots,d\}$, $(i,(i+1))\in E(I)$.  
  % 
  % 
  % 
  % $I$ has maximum out-degree $3$ and each directed edge $ij\in E(I)$ has $j < i$.
  % 
  % 
  % 
  % 
