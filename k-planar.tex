...
\begin{proof}
  ...
  \]
  We are now very close to \eqref{one-round}, which asks for the number of $\delta\in\{d_1,\ldots,d_0-1\}$ such that there exists a $\delta$-walk.
  The preceding formula counts $\delta=d_0$, but does not count $\delta=d_1$.  These two effects cancel each other, thus establishing \eqref{one-round}.
