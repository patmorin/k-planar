...
A \defin{layering} of a graph $G$ is a sequence $\mathcal{L}=\langle L_0,L_1,\ldots\rangle $ such that $\{L_0,L_1,\ldots\}$ is a partition of $V(G)$ 
\note{DW}{just write ``is an ordered partition $\mathcal{L}=\langle L_0,L_1,\ldots\rangle $ such that for every edge ...''}
and for every edge $vw\in E(G)$, if $v\in L_i$ and $w\in L_j$ then $|j-i|\leq 1$.  For any partition $\PP=\{S_1,\ldots,S_p\}$ of $V(G)$, a \defin{quotient graph} $H=G/\PP$ has a $p$-element vertex set $V(H)=\{x_1,\ldots,x_p\}$ and $x_ix_j\in E(H)$ if and only if there exists an edge $vw\in E(G)$ such that $v\in S_i$ and $w\in S_j$. To highlight the importance of the quotient graph $H$, we call $\PP$ an \defin{$H$-partition} and write this concisely as $\PP=\{S_x : x\in V(H)\}$ so that each element of $\PP$ is indexed by the vertex it creates in $H$ \note{DW}{``is indexed by the corresponding vertex in $H$''.}
...
\citet{DJMMUW20} showed that every planar graph has an $H$-partition of layered width $1$ for some planar graph $H$ of treewidth at most $8$ (improved to $6$ in \cite{UWY}). The proof is constructive and gives a simple quadratic-time algorithm for finding the corresponding partition and layering. At the core of their work is the elegant proof by \citet{PS21} of the following result:
...
We begin with a standard technique that allows us to work with a normalised tree-decomposition whose tree has same vertex set as the graph it decomposes:
...
% Apply \cref{generalized-tripod} to $G$ and let $\mathcal{L}$, $\mathcal{Y}$, $\mathcal{T}$, $T$, $H_x$, and $G_x$ be defined as in \cref{generalized-tripod}, where the partition $\mathcal{Y}$ has width $\ell$ with respect to the layering $\mathcal{L}$.

For a node $x\in V(T)$, we say that a shortcut $P\in\SS$ \defin{crosses} $x$ if $Y_x$ contains an internal vertex of $P$.  In other words, if $P=v_0,\ldots,v_r$ and $\{v_1,\ldots,v_{r-1}\}\cap Y_x\neq\emptyset$.  We say that a vertex $v\in V(G)$ \defin{participates} in $x$ if $v\in Y_x$, or $\SS$ contains a shortcut $P$ with $v\in V(P)$ and $P$ crosses $x$.  For each $v\in V(G)$, let $a(v)$ denote the lowest common $T$-ancestor of all the nodes $x\in V(T)$ in which $v$ participates.

% \begin{clm}
%   For each $v\in V(G)$, $v$ participates in $a(v)$.
% \end{clm}
% 
% \begin{proof}
...
% 
%   Let $X_H:=\{x\in V(H): Z\cap Y_x\neq\emptyset\}$.  Therefore $X_H$ contains every every node in which $v$ participates.  The connectivity of $G[Z]$ implies that $H[X_H]$ is connected. The fact that $\mathcal{T}$ is a normalized tree decomposition of $H$ implies that some node $a\in X_H$ is a $T$-ancestor of every node in $X_H$.  Thus, $v$ participates in the node $a=a(v)$ that is the lowest common $T$-ancestor of every node in which $v$ participates, as required.  
% \end{proof}


% \begin{proof}
%   Let $a(v)$.  If $v\in Y_{a(v)}$ then $v$ participates in $a(v)$, by definition, and we are done.  Otherwise, there are two nodes $x,y\in V(T)$ in which $x$ participates and whose lowest common $T$-ancestor is $a(v)$.
% 
%   There are two cases to consider:  
%   \begin{compactenum}
%     \item If $v\in Y_x$ then $v\not\in Y_y$.  Since $v$ participates in $Y_y$, there exist a shortcut $P:=v_0,\ldots,v_r$ in $\SS$ with $v=v_\alpha$ and $v_\beta\in Y_y$ for some $\alpha\in\{0,\ldots,r\}$ and $\beta\in\{1,\ldots,r-1\}$.  Without loss of generality, $\alpha < \beta$.
% 
%     For each $i\in\{0,\ldots,r\}$, let $Y_{x_i}$ be the part of $\mathcal{Y}$ that contains $v_i$.  Since $G[\{v_\alpha,\ldots,v_\beta\}]$ is connected, $H[\{x_\alpha,\ldots,x_\beta\}]$ connected.  Since $\mathcal{T}$ is a normalized tree decomposition of $H$, $x_j$ is a $T$-ancestor of every node in $x_\alpha,\ldots,x_\beta$ for some $j\in\{\alpha,\ldots,\beta\}$.      In particular $x_j$ is a $T$-ancestor of $x=x_\alpha$ and $y=x_\beta$, so $x_j$ is a $T$-ancestor of $a(v)$.  Since $v_j\in V(P)$ and $v_j\in Y_{x_j}$ this implies that $v$ participates in $x_j$.  Therefore $v$ participates in a $T$-ancestor of $a(v)$, which can only mean that $v$ participates in $a(v)$.
% 
%     \item If $v\not\in Y_x$ and $v\not\in Y_y$ then there are shortcuts $P_1,P_2\in\SS$ with $v\in V(P_1)$, $v\in V(P_2)$ where $P_1$ has an internal vertex $w_1\in Y_x$ and $P_2$ has an internal vertex $w_2\in Y_y$.    
% 
% 
%     .  Since $v$ participates in $Y_y$, there exist a shortcut $v_0,\ldots,v_r$ in $\SS$ with $v=v_\alpha$ and $v_\beta\in Y_y$ for some $\alpha\in\{0,\ldots,r\}$ and $\beta\in\{1,\ldots,r-1\}$.  Without loss of generality, $\alpha < \beta$.
% 
%     For each $i\in\{0,\ldots,r\}$, let $Y_{x_i}$ be the part of $\mathcal{Y}$ that contains $v_i$.  Since $G[\{v_\alpha,\ldots,v_\beta\}]$ is connected, $H[\{x_\alpha,\ldots,x_\beta\}]$ connected.  Since $\mathcal{T}$ is a normalized tree decomposition of $H$, $x_j$ is a $T$-ancestor of every node in $x_\alpha,\ldots,x_\beta$ for some $j\in\{\alpha,\ldots,\beta\}$.  In particular $x_j$ is a $T$-ancestor of $x=x_\alpha$ and $y=x_\beta$, so $x_j$ is a $T$-ancestor of $a(v)$. Since $v_j\in Y_{x_j}$, and $v\not\in Y_{a(v)}$ this implies that $v$ participates in $Y_{x_j}$.  Therefore $v$ participates in a $T$-ancestor of $a(v)$, which means that $v$ participates in $a(v)$.
% 
% 
%   \end{compactenum}
% \end{proof}
...
% We let $X_v$ denote the set of nodes $x\in V(T)$ such that $v$ participates in $x$.
...
% 
% 
%   Let $X_H:=\{x\in V(H): Z\cap Y_x\neq\emptyset\}$.  The connectivity of $G[Z]$ implies that $H[X_H]$ is connected.  This implies that $T':=T[]
% 
% 
% 
...
% \note{PM}{Same comment: Yes, these claims are definition checking.  We'll see if the experimental notation change helps.}

% \begin{clm}
% 	\label{s-subset}
%   For every $x\in V(T)$, 
%   % $S_x\subseteq V_x$.
%   $S_x\subseteq \bigcup_{x'\in V(T_x)} Y_{x'}$.
% \end{clm}
% 
% \begin{proof}
%   Let $v$ be any vertex in $S_x$ (which implies that $a(v)=x$) and let $Y_y$ be the part of $\mathcal{Y}$ that contains $v$.  By definition, $v$ participates in $y$ so by definition $a(v)$ is a $T$-ancestor of $y$, so $y\in V(T_{a(v)})$.  Therefore, $v\in Y_y\subseteq \bigcup_{x'\in V(T_x)} Y_{x'}$, as required.
% \end{proof}
...
\begin{proof}
  ...
  Suppose, without loss of generality, that $v_0$ participates in $x_i$.  Then $a(v_0)=x$ is a $T$-ancestor of $x_i$ which a $T$-ancestor of $x_r$.  Finally $a(v_r)=y$ is a $T$-ancestor of $x_r$.  Therefore, both $x$ and $y$ are contained in the path from the root of $T$ to $x_r$, so one is a $T$-ancestor of the other. 
  % 
  % 
  % is a lazy walk in $H$.
  % 
  % Let $P:=v_0,\ldots,v_r$ (where $v_0=v$ and $v_r=w$)
  % Suppose, for the sake of contradiction that neither $x$ nor $y$ is an ancestor of the other.  Then
  % 
  % 
  % 
  % 
...
% 
% \note{PM}{I also found the steps in this proof tedious. Unfortunately, when I tried to give a simple explanation for it in Banff, both Piotr and Torsten remained unconvinced until we opened the paper and walked through the steps of the proof together.  I suspect the referee who can see through these proofs if one of the authors of \cite{PS21}.}
...
\begin{proof}
  ...
  \cref{i-ancestor} ensures that, for every edge $ax'\in E(J)$, $a,x'\in C_{x'}$.  The connectivity of $T[a]:=T[\{x\in V(T):a\in C_x\}]$ follows from the fact that, for every node $x'\in T[a]$, every node $x$ on the path in $T$ from $x'$ to  $a$ is also a node of $T[a]$.  Therefore $(C_x:x\in V(T))$ is indeed a tree-decomposition of $J$.  
  
  It remains to show that each bag in our tree decomposition has size at most $\binom{k+t}{t}$.  We will do this by appealing to an elegant result of \citet{PS21}. 
  % on the number of $k$-reachable vertices. in a directed graph obtained by directing the edges of a supergraph of $H$.  \
  Specifically, let $H^+$ denote the supergraph of $H$ that contains the edge $xy$ if and only if some bag $B_z\in\mathcal{T}$ contains both $x$ and $y$ (i.e., $H^+$ is the chordal closure of $H$ with respect to $\mathcal{T}$).  Let $\overrightarrow{H}^+$ be the directed graph obtained by directing each edge $xy$ of $H^+$ in the direction $\overrightarrow{xy}$ so that $y$ is a $T$-ancestor of $x$.  \citet[Lemma~13]{PS21} show that, for any $x\in V(\overrightarrow{H}^+)$, the number of vertices of $\overrightarrow{H}^+$ that can be reached by a directed path that originates at $x$ and has length at most $k$ is at most $\binom{k+t}{t}$.

  Consider an arbitrary node $x\in V(T)$ and suppose that some strict $T$-ancestor $a$ of $x$ is contained in $C_x$.  We will show that $\overrightarrow{H}^+$ contains a directed path from $x$ to $a$ of length at most $k$.  Since there $a$ a strict $T$-ancestor of $x$, $a\neq x$ so any path from $x$ to $a$ has length at least $1$. By \cite[Lemma~13]{PS21}, this implies that the number of strict $T$-ancestors of $x$ contained in $C_x$ is at most $\binom{k+t}{t}-1$ and therefore $|C_x|\le\binom{k+t}{t}$.
  
  Since $a\in C_x$, there exists a $T$-descendant $x'$ of $x$ such that $ax'\in E(J)$.  Therefore, there exists a shortcut $v_0,\ldots,v_r$ in $\SS$ with $v_0\in S_{x'}$ and $v_r\in S_a$.  For each $i\in\{0,\ldots,r\}$, let $Y_{x_i}$ be the part of $\mathcal{Y}$ that contains $v_i$.  Since $v_0,\ldots,v_r$ is a path in $G$, $x_0,\ldots,x_r$ is a lazy walk in $H$.\footnote{A \defin{lazy walk} is a graph $G$ is a walk in the pseudograph obtained by adding a self-loop at each vertex of $G$.}  Let $j$ be the minimum integer such that $x_j$ is a strict $T$-ancestor of $x$.  Since $x'=x_0$ is a $T$-descendant of $x$ and $a=x_r$ is a strict $T$-ancestor of $x$, $j\in\{1,\ldots,r\}$ and $x_{j-1}$ is a $T$-descendant of $x$ (possibly $x_{j-1}=x$).  Since $x_{j-1}x_j\in E(H)$, $x_j \in B_{x_{j-1}}$ and therefore $x_j\in B_x$.  Therefore $xx_j\in E(H^+)$.  
  
  Therefore $W:=y_0,\ldots,y_s:=x,x_j,x_{j+1},\ldots,x_r$ is a lazy walk in $H^+$ of length $r\le k$.  For each $j\in \{0,\ldots,s\}$, let $z_j$ be the lowest common $T$-ancestor of $y_0,\ldots,y_j$.  By using the same reasoning used to conclude that $xx_j\in E(H^+)$ we conclude that $z_{i-1}=z_i$ or $z_{i-1}z_i\in E(H^+)$ for each $i\in\{1,\ldots,s\}$, so $z_0,\ldots,z_s$ is a directed lazy walk in $\overrightarrow{H}^+$.  Finally, let $\overrightarrow{W}$ be the path in $\overrightarrow{H}^+$ obtained by removing removing duplicates from $z_0,\ldots,z_s$.  Then $\overrightarrow{W}$ is a directed path in $\overrightarrow{H}^+$ from $x$ to $a$ of length $s\le r\le k$.  This completes the proof.
\end{proof}
...
For any integer $d\ge 3$, a (multi)graph $G$ is \defin{$d$-framed} if it admits a drawing in the plane \note{DW}{Use the language of ``embedded graph''} having a biconnected plane spanning (multi) subgraph $G_0$, whose facial cycles have length at most $d$,  and such that the interior of each edge in $E(G)\setminus E(G_0)$ is contained in the interior of some face of $G_0$. The graph $G_0$ is called the \defin{frame} of $G_0$. This definition (for simple graphs) was introduced by \citet{BDGGMR}. Every $d$-framed multigraph can be described by a $(2,\binom{d}{2})$-shortcut system applied to a planar multigraph (by adding a vertex inside each face $F$ adjacent to the vertices of $F$). Thus \cref{ShortcutProduct} is applicable. However, for framed graphs we prove the following stronger result, where the treewidth bound on $H$ is best possible.\footnote{A multigraph $G$ is \defin{contained} in a graph $G'$ if the simple graph underlying $G$ is isomorphic to a subgraph of $G'$.}
...
This result is used below to obtain product structure theorems for 1-planar multigraphs and for map graphs. 
% \todo{Move this comment to the section on $1$-planar graphs?} 
For simple 1-planar graphs it is essential that we allow frames with parallel edges. 
...
\note{PM}{Consider a change of notation:   Replace $F$ with $C$.  (I was using $F$ before to be consistent with the previous section, but that notation is gone now.)} \note{DW}{I agree.}
...
\note{DW}{We need to define ``closure''}
...
\begin{lem}
	...
	\begin{compactenum}
		...
		\begin{compactenum}
			...
			\note{DW}{suggested revision: ``$P_i$ has a partition $\{X_i,Y_i\}$ where $|X_i|\le d-3$ and $|Y_i\cap L_j| \le 3$ for each $j\ge 0$.''}
		\end{compactenum}
	...
	\begin{compactenum}[(i)]
		...
		\item For each $x\in V(H)$, $S_x=Y_x\cup X_x$ where $|X_x|\le d-3$ and $|Y_x\cap L_j|\le 3$ for each $j\ge 0$ \note{DW}{suggested revision, ``$S_x$ has a partition $\{X_x,Y_x\}$ where $|X_x|\le d-3$ and $|Y_x\cap L_j|\le 3$ for each $j\ge 0$''};
...
\begin{proof}
	This proof is similar to the proof of Lemma~13 by \citet{DJMMUW20}, but is complicated by several factors.  In particular, the reader should keep in mind that $G$ and $G'$ are multigraphs and that the cycle $F$ may consist of two vertices and two (parallel) edges.

	The proof is by induction on the number of inner faces of $N'$.  A particular easy case occurs when $V(N')=V(F)$, for which we can simply take 
	\note{DW}{Suggested revision, ``In the base case, $N'$ has only one inner face, and we can simply take ...} 	
	$\PP:=\{P_1,\ldots,P_k\}$ and verify that the preconditions of the lemma (the `setup') ensure that $\PP$ satisfies the requirements (i)--(iv):
	\begin{compactenum}[(i)]
...
\note{DW}{Now assume that $N'$ has at least two internal faces.}
	...
	\begin{compactenum}
		\item If $k=1$ then let $\tau:=v_1,\ldots,v_p$ be an inner face of $N'$ that contains at least one edge $v_1v_2$ of $F$ on its boundary.
	...
	Let $S:=V(\overline{S})\setminus V(F)$, let $Y:=(V(Q_1)\cup V(Q_2)\cup V(Q_3))\setminus V(F)$, and let $X:=S\setminus Y$. Observe that, for each $i\in\{1,2,3\}$ and each $j\ge 0$, $|V(Q_i)\cap L_j|\le 1$.  Therefore $|Y\cap L_j|\le 3$ for each integer $j\ge 0$. Furthermore, $|X|\le p-3\le d-3$, since $v_1,v_2,v_3\in Y\cup V(F)$. Therefore, using $S:=Y\cup X$ as a part in the partition $\PP$ will not violate condition~(ii) \note{DW}{Use ``satisfies'' instead of ``will not violate''}.
...
\note{PM}{See my attempt below.}
% \note{PM}{I don't know what to say without explaining that $4$-framed (simple) graphs only contain $3$-connected $1$-planar graphs.  Vida's example shows that there is a $1$-plane embedding of a graph that can't be augmented into a $4$-framed graph, but I haven't checked if it can't be drawn some better way. And I also don't know if the Bekos claim that $8$-framed (smple) graphs contain all $1$-planar graphs is proven anywhere and, if so, is it tight?}
...
\begin{proof}[Proof of \cref{1_planar_is_4_framed}]
...
\note{PM}{I have no idea if this is known.  If so, it's probably not stated in terms of frames.}
\begin{lem}
