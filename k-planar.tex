...
For any vertex $v\in V(G)$ define $A_v=\bigcap\{vw\in E(G)} A_{vw}$.  $K[A_v]$ is a vertical path in $K$ that contains the root.  We define $a(v)$ to be the lower endpoint of $K[A_v]$.

For every node $x\in V(K)$, we define the \emph{separator} of $X$ as
\[
...
Next we describe a tree decomposition $(B_x:x\in V(K))$ of $H$.  We deliberately use the same node set $V(H)=V(K)$ here, because the subtree $K[x]:=K[\{z:x\in B_z\}]$ will be rooted at $x$.  We claim that, for every edge $xy\in V(H)$, one of $x$ or $y$ is a $K$-ancestor of the other.
Indeed, since $xy\in E(H)$, there exists an edge $vw\in E(G)$ such that $v\in S_x$ and $w\in S_y$.  By definition, $x\in A_v\subseteq A_{vw}$ and $y\in A_w\subseteq A_{vw}$.  Since $K[A_{vw}]$ is a vertical path in $K$, it follows that one of $x$ or $y$ is a $K$-ancestor of the other. 

To describe our tree decomposition we define, for each $y\in V(H)$, the subtree $K[y]=K[\{x:y\in B_x\}]$.  The subtree $K[y]$ contains $y$.  Additionally, for every edge $xy\in E(H)$, where $y$ is an ancestor of $x$, $K[y]$ includes the path from $y$ to $x$ in $K$.  That this produces a tree-decomposition is due to the fact, established in the preceding paragraph, that for every edge $xy$ of $H$, $x$ is a $K$-ancestor of $y$ (or vice-versa).  What remains is to upper bound the width of this decomposition.
