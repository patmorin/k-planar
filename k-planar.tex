...
\begin{proof}[Proof of \cref{d_framed_product_stucture}]
    Let $G$ be a $d$-framed multigraph with frame $G'$\todo{Deal with the case where $G'$ has no triangular face.} and assume that the outer face of $G'$ has exactly three vertices $v_1,v_2,v_3$ and that no edges of $G$ are in the outer face of $G'$.  Add a vertex $r$ adjacent to $v_1$, $v_2$, and $v_3$ to obtain a $d$-framed multigraph $\overline{G}$ with frame $\overline{G}'$.

	Let $T$ be a BFS tree of $\overline{G}'$ rooted at $r$ and let $\langle L_0,L_1,\ldots,\rangle$ be the resulting BFS layering.  Now apply \cref{induction} with $G=\overline{G}$, $\overline{G}'$, $F$, and $P_i=\{v_i\}$ for each $i\in\{1,2,3\}$.  This gives an $H$-partition $\mathcal{P}:=\{S_x\cup\{v_x\}:x\in V(H)\}$ of $\overline{G}-\{r\}\supseteq G$ in which $H$ has a $3$-simple tree decomposition.
