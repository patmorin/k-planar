...
\begin{proof}[Proof of \cref{kPlanarProduct}]
	Let $G$ be a $k$-plane graph.  We assume, for ease of exposition, that any point $p\in\R^2$ is involved in at most one crossing $(p,vw,xy)$ of $G$. This assumption is justified since it can be enforced by a slight deformation of the edges of $G$ and the resulting (deformed) graph is also $k$-plane.
...
\note{DW}{I will rewrite the intro to this subsection once we decide what our results are.}
...
\begin{lem}
	\label{NewLemma}
Let $G_0$ be a 2-connected plane multigraph. Let $T$ be a spanning tree of $G_0$ rooted at some vertex $r$ on the boundary of the outerface of $G_0$. Let $C = [P_1,\dots,P_k]$ be a cycle in $G_0$ with $r$ in the exterior of $C$, where $k\in\{1,2,3\}$ and $P_1, \dots, P_k$ are bipods in $G_0$. Let $G:=G_0\blah{C}$. Then $G$ has a partition $\PP$ into tripods with respect to $T$ such that $P_1,\dots,P_k\in\PP$, and $G / \PP$ has a tree-decomposition $\mathcal{T}$ of width at most 3 such that for every facial cycle $F$ of $G$ there is a bag $B$ of $\mathcal{T}$ such that every part of $\PP$ that intersects $F$ is in $B$.
\end{lem}
...
Thus $V(G_j)\setminus V(Z_j) \subsetneq V(G)\setminus V(C)$, and we may apply induction to $G_j$.  Thus $G_j$ has a partition $\PP_j$ into tripods with respect to $T$ such that $R_{i,j}^+, R_{i+1,j}^-, L_{i,j}\in\PP_j$, and 	$G_j / \PP_j$ has a tree-decomposition  $\mathcal{T}_j=\{B_x:x\in V(D_j)\}$ of width at most 3 such that for every facial cycle $F'$ of $G_j$ there is a bag $B$ of $\mathcal{T}_j$ such that every part of $\PP_j$ that intersects $F'$ is in $B$.
...
\begin{cor}
	\label{NewMapGraph}
	Every $d$-map graph is contained in $H \boxtimes P \boxtimes K_{ d  \floor{d/2} }$ for some graph $H$ with $\tw(H) \leq 3$ and for some path $P$.
\end{cor}
