...
\begin{proof}
  ...
  Clearly, $\mathcal{Y}:=(Y_x:x\in V(T))$ is a partition of $V(G)$.  Furthermore, for any $i\in\in\{1,\ldots,h\}$ 
  \[  |Y_x| = \sum_{y\in C_x}|Q_y\cap V_i|\le |C_x|\ell \le t\ell ,
  \] 
  ...
  Since $\mathcal{Y}$ is a partition, $w\in Y_{a}$ for some $a\in V(T)$, $a\neq x$.  Since $v\in Y_{x'}$ and $w\in Y_a$, $q(v)=x'$ and $q(w)=a$, so $x'\in Q_v$ and $w\in Q_a$.  Therefore $H$ contains the edge $ax'$.
  
  
  By \tref{ancestor-edge}, $a$ is an ancestor of $x'$  
\end{proof}
