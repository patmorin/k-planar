...
Let $T$ and, for each $x\in V(T)$, $Y_x$, $V_x$, $F_x$, and $N_x$ be a decomposition obtained by applying \thmref{generalized-tripod} to $G$, let $H:=G/\{Y_x:x\in V(T)\}$ denote the resulting quotient graph, and let $(B_x:x\in V(T))$ be a tree-decomposition of $H$ given by \lemref{nice-decomposition}.  For simplicity, we bypass the graph $H'$ and assume, from the outset, that $V(H)\subseteq V(T)$.

For two nodes $v,w\in V(G)$, a $(k,d)$ \emph{$vw$-longcut} is a walk $W=v_0,\ldots,v_{r}$ in $G$ of length $r\le k$ with $v_0=v$, $v_r=w$, in which, for each $i\in\{1,\ldots,r-1\}$, the degree of $v_i$ in $G$ is at most $d$.  Observe that every $(k,d)$ $vw$-shortcut is also a $(k,d)$ $vw$-longcut and that $G$ contains a $(k,d)$ $vw$-shortcut if and only if it contains a $(k,d)$ $vw$-longcut. 

For a node $x\in V(T)$, we say that a $(k,d)$ $vw$-longcut $W$ in $G$ \emph{crosses} $x$ if $Y_x$ contains an internal vertex of $W$.  We say that a vertex $v\in V(G)$ \emph{participates} in $x$ if $v\in Y_x$ or $G$ contains a $vw$-longcut that crosses $x$.

\begin{clm}
  For every vertex $v\in V(G)$, there is a (unique) node $a(v)\in V(T)$ such that $v$ participates in $a(v)$ and $a(v)$ is a $T$-ancestor of every node $x$ in which $v$ participates. 
\end{clm}
...
\begin{proof}
  ...
  Let $X_H:=\{x\in V(H): X\cap Y_x\neq\emptyset\}$.  The connectivity of $G[X]$ implies that $H[X_H]$ is connected.  
...
\[  
     S_x := \{v\in V(G): a(v)= x\} \enspace .
\]
Observe that $\mathcal{S}:=(S_x : x\in V(T))$ is a partition of $V(G)$.  We let $I:=G^{k,d}/\mathcal{S}$ denote the resulting quotient graph and we let $V(I)\subseteq V(T)$ in the obvious way, so that each $x\in V(I)$ is the vertex obtained by contracting $S_x$ in $G^{k,d}$. (Nodes $x\in V(T)$ with $S_x=\emptyset$ do not contribute a vertex to $I$.)
...
\begin{proof}
  ...
  By \yref{y-subsets}, $Y_x\subseteq V_x$, so $v\not\in Y_x$.  Therefore, $G$ contains a path $v_0,v_1,\ldots,v_{r}$ such that $r <k$, $v_0=v$, $v_r\in Y_x$ and the degree of $v_i$ in $G$ is at most $d$ for each $i\in\{1,\ldots,r\}$.  Since $v\not\in V_x$ and $v_r\in V_x$, \yref{separator}, implies that $v_i\in F_x$ for some $i\in\{0,\ldots,r-1\}$. Now \yref{ancestor-edge} implies $v_i\in Y_a$ for some strict $T$-ancestor $a$ of $x$.  But the existence of the path $v_0,\ldots,v_i$ in $G$ implies that $v=v_0\in S_{a'}$ for some ancestor $a'$ of $a$, contradicting the assumption that $v\in S_x$.
\end{proof}

Next we show that $\mathcal{S}$ has small layered width with respect to $\mathcal{L}$:

\begin{clm}\clmlabel{general-width}
  For each $i\in\{1,\ldots,h\}$ and each $x\in V(I)$, $|S_x\cap V_i|\le (2k-1)\ell d^k$.  
\end{clm}

\begin{proof}
  Consider some vertex $v\in S_x\cap L_i$.  Since $v\in S_x$, $G$ contains a path $v_0,\ldots,v_r$ such that $r<k$, $v_0\in Y_x$, $v_r=v$, and the degree of $v_i$ in $G$ is at most $d$, for each $i\in\{0,\ldots,r-1\}$.  Note that $v_0$ is one of the at most $(2k-1)\ell$ vertices in $Z=\bigcup_{j=-k+1}^{k-1}(V_i\cap Y_x)$. For each $w\in Z$, the number of such paths originating from $w$ is at most $\sum_{r=0}^{k-1} d^{r}< d^k$ (for $d\ge 2$). Therefore, $|S_x\cap L_i|\le |Z|d^k\le (2k-1)\ell d^k$.
  % \end{enumerate}
\end{proof}
...
\begin{proof}
  ...
  \end{enumerate}
  In the former case we define $P_{ww'}$ to be the path of length 0 that contains only $w=w'$.  Let $w''$ denote the first vertex of $P_{ww'}$ contained in $Y_{\delta}$.
