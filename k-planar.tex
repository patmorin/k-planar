...
\begin{lem}
	...
	\begin{compactenum}[(i)]
        ...
        \item For each $x\in V(H)$, $S_x=Y_x\cup X_x$ where $|X_x|\le 1$ and $|Y_x\cap L_j|\le 3$ for each $j\ge 0$;
        \item $H$ has a plane embedding in which $x_1,\ldots,x_k$ appear on the outer face;
		\item $\tw(H)\le 3$.
	\end{compactenum}
\end{lem}
...
\begin{proof}
  ...
  \begin{compactenum}[(i)]
    ...
    \item For each $x\in V(H)$, let $Y_x:=S_x$ and $X_x=\emptyset$.  Then 4(a) ensures that $|Y_x\cap L_j|\le 3$ for each integer $j\ge 0$.
    \item $H$ has $k\le 3$ vertices and any graph on $k\le 3$ vertices has a plane embedding with all its vertices on the outer face.
    \item The trivial tree decomposition of $H$ that places $x_1,\ldots,x_k$ in one bag has width $k-1\le 2\le 3$.
  \end{compactenum}
  From this point on, we assume that $N$ has at least one inner vertex.

  The $H$-partition $\mathcal{P}$ will include $P_1$, $P_2$ , $P_3$ and a (possibly empty) part $S$ that is defined by three vertices $v_1v_2v_3$ that belong to a common inner face $\tau$ of $N'$.  Depending on the value of $k$, there are two cases to consider:
  \begin{compactenum}
  ...
  We can now define the vertices in $S$.  For each $i\in\{1,2,3\}$, let $Q_i$ be the shortest path, in $T$, from $v_i$ to a vertex in $V(F)$.  Let $\overline{S}$ denote the subgraph of $N'$ consisting of vertices and edges of $Q_1$, $Q_2$, $Q_3$, and $\tau$.  Let $S:=V(\overline{S})\setminus V(F)$, let $Y:=(V(Q_1)\cup V(Q_2)\cup V(Q_3))\cap S$, and let $X:=S\setminus Y$.
  Observe that, for each $i\in\{1,2,3\}$ and each $j\ge 0$, $|V(Q_i)\cap L_j|\le 1$.  Therefore $|Y\cap L_j|\le 3$ for each integer $j\ge 0$. Furthermore, $|X|\le p-3\le 1$, since $v_1,v_2,v_3\in Y\cup V(N)$. Therefore, taking $Y:=S\cup\{v\}\setminus V(F)$ as a part in the partition $\PP$ will not violate condition~(ii).
  ...
  Let $F_1,\ldots,F_m,\tau$ be the inner faces of $M$.   By our choice of $\tau$, the graph $M$ is $2$-connected.  Furthermore, $V(F_i)$ can be partitioned into at most three sets $P_{i,1}$, $P_{i,2}$, and $P_{i,3}$ where $P_{i,1}\subset Y$, $P_{i,2}\subseteq P_b$, and $P_{i,3}\subseteq P_c$ for some $b,c\in\{1,\ldots,k\}$.
  ...
  \begin{compactenum}[(i)]
    \item By construction $\mathcal{P}$ contains parts $P_1,\ldots,P_k$.

    \item  By Assumption~4(b) none of the parts $P_1,P_2,P_3$ violate condition~(ii).  We have already argued that the part $S$ does not violate condition~(ii).  The inductive hypothesis ensures that each of the remaining parts satisfies condition~(ii).

    \item We only consider the (most difficult) case where $k=3$, the cases $\in\{1,2\}$ are handled easily. Let $x_0$ be the vertex of $H$ obtained from contracting $Y$ and, for each $j\in\{1,\ldots,3\}$, let $x_j$ denote the vertex of $H$ obtained from contracting $P_j$.  In this case the vertices $x_0,\ldots,x_3$ form a clique in $H$, which we embed with $x_1,\ldots,x_3$ on the outer face, as required.
    ...
    Therefore, by the inductive hypothesis, we can embed each such graph $H_i :=N_i/\mathcal{P}_i$ so that the three vertices on its outer face correspond to the appropriate inner face of the clique on $x_0,\ldots,x_3$.  Any remaining graphs $H_i$ have outer faces with at most two vertices on their outer faces corresponding to at most two vertices of $x_0,\ldots,x_3$.  These can easily be embedded, one a time, in some inner face that has the edge $x_ix_j$ on its boundary.
    ...
    The inductive hypothesis ensures that $H_i:=N_i/\mathcal{P}_i$ has a tree decomposition $(B_z:z\in V(T_i))$ of width at most $3$.  Let $a_i,b_i,c_i\in V(H_i)$ be the vertices obtained by contracting $P_{i,1}$, $P_{i,2}$, and $P_{i,3}$, respectively. Since $a_i,b_i,c_i$ form a clique in $H_i$, some bag $B_{z_i}$ contains $a_i$, $b_i$, and $c_i$.
    ...
    Using the fact that each component of $G-V(M)$ is contained in a single face of $M$, it is now straightforward to verify that, by doing this for each $i\in\{1,\ldots,m\}$ we obtain a width-$3$ tree decomposition of $N$.  \qedhere
  \end{compactenum}
...
\begin{proof}[Proof of \cref{1-planar}]
	...
	Let $G'$ be the plane graph obtained by removing each pair of crossing edges from $\overline{G}$ and let $T$ be a BFS tree of $G'$ rooted at $r$ and let $\langle L_0,L_1,\ldots,\rangle$ be the resulting BFS layering.  Now apply \cref{induction} with $G=\overline{G}$, $G'$, $F$, and $P_i=\{v_i\}$ for each $i\in\{1,2,3\}$.  This gives an $H$-partition $\mathcal{P}:=\{S_x\cup\{v_x\}:x\in V(H)\}$ of $\overline{G}-\{r\}\supseteq G$ in which $H$ is planar and has treewidth at most 3.

	Use the layering $\mathcal{L}=\langle L_0',L_1'\ldots\rangle$ where $L_i'=L_{2i}\cup L_{2i+1}$ for each integer $i\ge 0$. That this is a layering of $G$ follows from the fact that $\dist_{G'}(v,w)\le 2$ for each edge $vw\in E(G)$.  Since $|L_j\cap Y_x|\le 3$ for each integer $j\ge 0$, $|L_i'\cap Y_x|\le 6$ and therefore $|L_i'\cap S_x|\le 7$ for each integer $i\ge 0$ and each $x\in V(H)$. The result follows from \cref{PartitionProduct}.
\end{proof}

We remark that our proof of \cref{1-planar} generalizes to the class of $d$-framed graphs \cite{BDGGMR} and even $d$-framed multigraphs.  An embedded multigraph $G$ is a \defin{$d$-framed multigraph} if it contains a plane subgraph $G'$ (the \defin{frame}) whose edges are not involved in any crossings and such that each pair of crossing edges in $G$ are contained in a face of $G'$ that has at most $d$ vertices on its boundary.  It is known that every $3$-connected $1$-planar graph is contained in some $4$-framed graph.  The discussion on the structure of $G$ and $G'$ that precedes \cref{induction} shows that every $1$-planar graph is contained in a $4$-framed multigraph.
The proof of \cref{induction} can be repeated for any edge-maximal $d$-framed multigraphs $G$ having frame $G'$. The proof in this case has only two differences:
\begin{compactitem}
...
\begin{thm}
\label{framed}
   Every $d$-framed multigraph is contained in $H\boxtimes P\boxtimes K_{3\lfloor d/2\rfloor + d -3}$ for some planar graph $H$ with treewidth at most 3 and for some path $P$.
\end{thm}
...
\note{PM}{I no longer agree about the restructuring. See my email essay for an explanation.}

\note{PM}{Apropos the extra clause.  It's related to the orange todo note (argue that $N'/\mathcal{P}=N/\mathcal{P}$).  This doesn't need to be in the statement of the lemma since, unlike \cref{NewLemma} this one gives an $H$-partition of $N$, not $N'$, but something like it needs to be in the proof.}
...
\note{DW}{I am pretty sure one can get treewidth 3 for $(g,1)$-planar graphs, combining \cref{framed} with the treewidth 3 result in \citep{DHHW}. I think the best place for this result is reference \citep{DHHW} since the present paper pre-dates \citep{DHHW} (and this addition will increase the significance of \citep{DHHW}). }
...
\note{DW}{I am pretty sure one can get treewidth 3 here using \cref{framed} and the treewidth 3 result in \citep{DHHW}. I think the best place for this result is reference \citep{DHHW} since the present paper pre-dates \citep{DHHW} (and this addition will increase the significance of \citep{DHHW}).}
...
Let $G$ be a 2-connected plane multigraph, in which every facial cycle has length in $\{3,\dots,d\}$ for some fixed integer $d\geq 3$. Then $\widehat{G}$ is contained in $H \boxtimes P \boxtimes K_{ d +3( \floor{d/2}-1) }$ for some graph $H$ with $\tw(H) \leq 3$ and for some path $P$.
\end{thm}
