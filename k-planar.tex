...
\begin{proof}
  For the tree-decomposition $(B_x:x\in V(T))$ of $I$ we use the same tree $T$ used in the tree-decomposition of $H$. For each node $x$ of $T$, $B_x$ contains $x$ as well as every $T$-ancestor $a$ of $x$ such that $I$ contains an edge $ax'$ where $x$ is a $T$-ancestor of $x'$ (including the possibility that $x=x'$).
  \clmref{i-ancestor} ensures that, for every edge $ax'\in E(I)$, $a,x'\in B_{x'}$ and the connectivity of $T[a]:=T[\{x\in V(T):a\in B_x]$ is obvious.  Therefore $(B_x:x\in V(T))$ is indeed a tree-decomposition of $I$.  What remains is to bound the size of the largest bag.
  
  Consider an arbitray node $x\in V(T)$ whose ancestors are $x_0,\ldots,x_{r-1},x_r=x$.  If $x_\delta\in B_x$, it is because $x_\delta x'\in E(I)$ for some $T$-descendant $x'$ of $x$.  This implies $G^{k,d}$ contains an edge $vw$ with $v\in S_{x'}$ and $w\in S_{x_i}$.  This implies that $G$ contains a $(k,d)$ $vw$-shortcut.  Since $w\in S_{x_i}$, $G$ contains a $(k,d)$ $ww'$-longcut $P_{ww'}$ that has an internal vertex in $w''\in Y_{x_i}$.
  
  Let $w_0,\ldots,w_p$ be the subpath of $P_{ww'}$ that begins at $w_0:=w$ and ends at $w_p:=w''$.  For each $i\in\{0,\ldots,p\}$, let $s_i=\max\{j: \{w_0,\ldots,w_i\}\subseteq V_{x_j}\}\}$.  Note that $s_0,\ldots,s_p$ is a non-increasing sequence with $s_0=r$ and $s_p=\delta$.  By \yref{ancestor-edge}, given $s_i$, there are at most $t+1$ possible values of $s_{i+1}$:  Either $s_{i+1}=s_i$ or $s_{i+1}$ is the index of one of the $t$ ancestors described in \yref{ancestor-edge}.  This immediately implies that $|B_x|\le \sum_{i=0}^{k-1} t^i < t^k$ for $t\ge 2$.  But we can do better....
\end{proof}
