...
We use the layering $\mathcal{L}=\{V_0,V_1,\ldots,V_h\}$ where $V_i=\{ v\in V(\Delta)\setminus\{r\}: \dist_{\Delta}(r,v)=i\}$ and $h=\max\{\dist_\Delta(r,v):v\in V(\Delta)\}$.  The fact that $T$ is a BFS tree ensures that $\mathcal{L}$ is, indeed, a layering of $\Delta-\{r\}$.  We use the vertex partition $\mathcal{P}=\{ S_x:=Y_x : x\in V(K)\}$.  \propref{partition} ensures that $\mathcal{P}$ is, indeed a partition of $V(\Delta)-\{r\}$.
...
In this section, $G$ is a 1-plane graph. We will construct a   To construct a layered $H$-decomposition of $G$. First, we augment $G$ as follows:
\begin{enumerate}
  \item Add edges to $G$ to obtain a multigraph $G^+$ that is edge-maximal in the following sense:  if any two vertices $v,w\in V(G^+)$ appear on a common face\footnote{The \emph{faces} of an embedded graph $G$ are the connected components of $\R^2\setminus \bigcup_{vw\in E(G)} vw$.  We say that a vertex $v\in V(G)$ appears on a face $F$ if $v$ is contained in the closure of $F$.} $F$ of $G^+$, then there is an edge $vw\in E(G^+)$ that is contained in the boundary of $F$.
  
  \item Add a complete graph $K_4$ that contains $G^{+}$ in one of its inner faces, $F_0$, and then add edges to the resulting graph to obtain a triangulation $G^{++}$.
\end{enumerate}

Note that, in contrast to \secref{k-planar}, neither $G^+$ nor $G^{++}$ is necessarily a plane graph; crossings that occur in $G$ also occur in $G^+$ and $G^{++}$.
...
The 1-plane graph $G^+$ has a plane triangulation $G^-$ as a subgraph that can be obtained by removing one spar from each kite in $G^+$.  Observe that, for any spar $vw\in E(G)\setminus E(G')$ that crosses $xy\in E(G^-)$, $G^-$ contains the path $vxw$ (and $vyw$).  It follows that $\dist_{G^-}(x,y)\le 2\cdot\dist_{G^+}(x,y)$ for any pair of vertices $x,y\in V(G)$.
