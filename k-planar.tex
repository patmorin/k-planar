...
Thus $V(G_j)\setminus V(Z_j) \subsetneq V(G)\setminus V(C)$, and we may apply induction to $G_j$.  Thus $G_j$ has a partition $\PP_j$ into tripods with respect to $T$ such that $R_{i,j}^+, R_{i+1,j}^-, L_{i,j}\in\PP_j$, and 	$G_j / \PP_j$ has a tree-decomposition  $\mathcal{T}_j=\{B_x:x\in V(D_j)\}$ of width at most 3 such that for every facial cycle $F'$ of $G_j$ there is a bag $B$ of $\mathcal{T}_j$ such that every part of $\PP_j$ that intersects $F'$ is in $B$. Since $C$ is a facial cycle of $G_j$, there is a bag $B_{u_j}$ of $G_j$ such that $P_1,\dots,P_k\in B_{u_j}$. 
