...
We now generalize \thmref{k-planar} to a more abstract setting.  For two sets $A$ and $B$, we use $A\ominus B := (A\cup B)\setminus(A\cap B)$ to denote the symmetric difference between $A$ and $B$.  We begin with a fairly standard lemma about normalized tree decompositions:
...
\begin{proof}
  ...
  Create a new tree $T$ from $T_0$ by adding a path of nodes $x_0,x_1,x_2,\ldots,x_{t'-1},x_{t'}=x$. Let $C_{x_0}:=\emptyset$ and, for each $i\in\{1,\ldots,t'\}$, $C_{x_i}:=\{v_1,\ldots,v_i\}$.  Note that $(C_x: x\in V(T))$ is a width-$(t-1)$ tree-decomposition of $H$. Let $T$ be rooted at $x_0$. Note that $(C_x:x\in V(T))$ satisfies Properties~\tref{rooted}, and \tref{diff}.  For the remaining properties some renaming is required, which we now describe.
  
  For each $v\in V(H)$, let $v'\in V(T)$ be the unique node such that $v\in C_{v'}$ and $v'$ has minimum $T$-depth.  The uniqueness of $v'$ follows from the fact that $T[v]:=T[\{x\in V(T):v\in B_x\}]$ is connected, so $v'$ is the root of $T[v]$.  Furthermore, for any pair of distinct vertices $v,w\in V(H)$, \tref{rooted} and \tref{diff} ensure that $v'\neq w'$ since, otherwise, $C_{v'}\ominus C_{x'}\supseteq \{v',w'\}$ where $x'$ is the parent of $v'=w'$.  Property~\tref{rooted} would ensure that $x'$ exists and Property~\tref{diff} would ensure that $|C_{x'}\ominus C_{v'}|\le 1$.
  
  For each $x\in V(T)$, let $B_x=\{v' : v\in C_x\}$.  Now define the graph $H'$ to have vertex set $V(H')=\{v':v\in V(H)\}$ and edge set $E(H')=\{v'w': vw\in E(H)\}$.  Obviously, $H'$ isomorphic to $H$ and $V(H')\subseteq V(T)$ and $(B_x: x\in V(T))$ is a width=$(t-1)$ tree-decomposition of $H'$.  The decomposition $(B_x: x\in V(T))$ inherits Properties~\tref{rooted} and \tref{diff} from $(C_x:x\in V(T))$ and satisfies Property~\tref{node-set} and \tref{subtree-root} by definition.
  
  To see that $(B_x:x\in V(T))$ satisifies Property~\tref{ancestor-edge}, observe that, if $vw\in E(H)$, then at least one of $v'$ or $w'$ is contained in $B_x$ for every node $x$ on the path from $v'$ to $w'$ in $T$.  If neither $v'$ nor $w'$ is an ancestor of the other, then some node $x$ on this path has $T$-depth less than that of $v'$ and $w'$.  If $v'\in B_x$ this contradicts the fact that $v'$ is the root of $T[v']$.  If $w'\in B_x$ this contradicts the fact that $w'$ is the root of $T[w']$.
\end{proof}
...
Before proving \thmref{generalized-tripod} we point two properties that are immediately implied by it:
...
Property~\yref{y-subsets} follows from the fact that $V_x$ is the union of several sets, one of which is $Y_x$.  Property~\yref{containment-i} follows from the definitions of $V_x$ and the fact that the subtree $V(T_x)\subseteq V(T_a)$. To show Property~\yref{containment-ii} first note that, by \yref{containment-i} it suffices to consider vertices in $w\in F_x=N_x\setminus V_x$. By definition, every vertex $w\in F_x$ is adjacent, in $G$, to a vertex $v\in V_x$.  By \yref{containment-i}, $v\in V_a$, so $w$ is either in $V_a$ or $w$ satisfies the condition $vw\in E(G)$, $v\in V_a$, and $w\not\in V_a$, so $w\in F_a$.  In either case $w\in N_a=V_a\cup F_a$.  Note that none of \yref{y-subsets}--\yref{containment-ii} depends on \yref{ancestor-edge} (which is important, since \yref{containment-i} is used to establish \yref{ancestor-edge} below).

\begin{proof}[Proof of \thmref{generalized-tripod}]
  Apply \lemref{nice-decomposition} to $I$ to obtain a tree-decomposition $(B_x: x\in V(T))$ of $I$ where $T$ is rooted at some node $x_0$ and that satisifies Properties~\tref{first}--\tref{last}. (We skip the introduction of the isomorphic graph $H'$ and assume from the outset that $V(I)\subseteq V(T))$.)
  ...
  To establish Property~\yref{ancestor-edge}, consider some vertex $w\in F_x$.  Since $w\in F_x$, there exists an edge $vw\in E(G)$ with $v\in V_x$ and $w\not\in V_x$.  Since $v\in V_x$, $v\in Y_{x'}$ for some $T$-descendant $x'$ of $x$ (possibly $x=x'$). Since $\mathcal{Y}$ is a partition, $w\in Y_{a}$ for some $a\in V(T)$, $a\neq x$.  Since $v\in Y_{x'}$ and $w\in Y_a$, $q(v)=x'$ and $q(w)=a$, so $x'\in Q_v$ and $w\in Q_a$.  Therefore $H$ contains the edge $ax'$.  By \tref{ancestor-edge}, $a$ is an ancestor of $x'$ or $x'$ is an ancestor of $a$.  Since $w\in Y_a\subseteq V_a$ and $w\not\in V_x\supseteq V_{x'}$, \yref{containment-i} rules out the possibility that $x'$ is a $T$-ancestor of $a$. Therefore, $a$ is a $T$-ancestor of $x$ which is a $T$-ancestor of $x'$.  Let $x'=z_0,\ldots,z_r=a$ be the path, in $T$ from $x'$ to $a$.  For each $i\in\{0,\ldots,r\}$, at least one of $a$ or $x'$ is in $C_{z_i}$.  However, $b(x')=x'$, so $x'$ is not contained in $C_{x_i}$ for any $i\in\{1,\ldots,r\}$.  Therefore $a\in C_{x_i}$ for each $i\in\{0,\ldots,r\}$.  In particular, $a$ is contained in $C_x$.
  Property~\yref{ancestor-edge} now follows from the fact that $|C_x|\le t$.
\end{proof}
...
Let $T$ and, for each $x\in V(T)$, $Y_x$, $V_x$, $F_x$, and $N_x$ be a decomposition obtained by applying \thmref{generalized-tripod} ot $G$, let $H:=G/\{Y_x:x\in V(T)\}$ denote the resulting quotient graph, and let $(B_x:x\in V(T))$ be a tree-decomposition of $H$ given by \lemref{nice-decomposition}.  For simplicity, we bypass the graph $H'$ and assume, from the outset, that $V(H)\subseteq V(T)$.

For two nodes $vw\in V(G)$, a $(k,d)$ \emph{$vw$-longcut} is a walk $W=v_0,\ldots,v_{r}$ in $G$ of length $r\le k$ with $v_0=v$, $v_r=w$, in which, for each $i\in\{1,\ldots,r-1\}$, the degree of $v_i$ in $G$ is at most $d$.  Observe that every $(k,d)$ $vw$-shortcut is also a $(k,d)$ $vw$-longcut and that $G$ contains a $(k,d)$ $vw$-shortcut if and only if it contains a $(k,d)$ $vw$-longcut. 

For a node $x\in V(T)$, we say that a $(k,d)$ $vw$-longcut $P$ in $G$ \emph{crosses} $x$ if $Y_x$ contains an internal vertex of $P$.  We say that a vertex $v\in V(G)$ \emph{participates} in $x$ if $v\in Y_x$ or $G$ contains a $vw$-longcut that crosses $x$.

\begin{clm}
  For every vertex $v\in V(G)$, there is a (unique) node $a(v)\in V(T)$ such that $v$ participates in $a(v)$ and $a(v)$ is a $T$-ancestor of every node in which $x$ participates. 
\end{clm}

\begin{proof}
  Consider the subset $X$ of vertices $v'\in V(G)$ such that $G$ contains a path from $v$ to $v'$ of length at most $k$ in which every vertex other than (possibly) $v$ has degree at most $d$.  Clearly $G[X]$ is connected and, by definition, $v$ participates in a node $x\in V(T)$ if and only if $X\cap Y_x\neq\emptyset$.

  Let $X_H:=\{x\in V(H): X\cap Y_x\neq\emptyset\}$.  The connectivity of $G[X]$ implies that $H[X_H]$ is connected.  Property~\tref{ancestor-edge} implies that, for every $xy\in E(H)$, one of $x$ or $y$ is a $T$-ancestor of the other. Choose $a(v)\in X_H$ to be any member of $X_H$ that does not have a strict $T$-ancestor in $X_H$.  Transitivity of the $T$-ancestor relationship, \tref{ancestor-edge}, and connectivity of $H[X_H]$ implies that $a(v)$ is a $T$-ancestor of every node $x\in X_H$, as required.
\end{proof}
...
\begin{proof}
  ...
  By \yref{y-subsets}, $Y_x\subseteq V_x$, so $v\not\in Y_x$.  Therefore, $G$ contains a path $v=v_0,v_1,\ldots,v_{r}$ such that $r <k$, $v_r\in Y_x$ and the degree of $v_i$ in $G$ is at most $d$ for each $i\in\{1,\ldots,r\}$.  Since $v\not\in V_x$ and $v_r\in V_x$, \yref{separator}, implies that $v_i\in F_x$ for some $i\in\{0,\ldots,r-1\}$. Now \yref{ancestor-edge} implies $v_i\in Y_a$ for some srict $T$-ancestor $a$ of $x$.  But the existence of the path $v_0,\ldots,v_i$ in $G$ implies that $v=v_0\in S_{a'}$ for some ancestor $a'$ of $a$, contradicting the assumption that $v\in S_x$.
\end{proof}

Next we show that $\mathcal{S}$ has small layered width with respect to $\mathcal{L}$:

\begin{clm}\clmlabel{general-width}
  For each $i\in\{1,\ldots,h\}$ and each $x\in V(I)$, $|S_x\cap V_i|\le t\ell(1 + 2(k-1)d^{k-1})$.  
\end{clm}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item $G$ contains a path $v=v_0,\ldots,v_r$ with $r<k$ such that $v_r\in Y_x$ and the degree of $v_i$ in $G$ is at most $d$.
    In this case, $v_r\in L_{i+j}\cap Y_x$ for some $j\in\{-k+1,\ldots,k-1\}$.
    For each $w\in L_{i+j}\cap Y_x$, the number of such paths originating from $w$ is at most $d^{k-1}$. Therefore, there are at most $t\ell(d-1)^{k-1}$ such vertices in $S_x\cap L_i$. \qedhere
  \end{enumerate}
\end{proof}
...
\begin{proof}
  Suppose, for the sake of contradiction, that neither $x$ nor $y$ is a $T$-ancestor of the other.  Since $xy\in E(I)$, $G^{k,d}$ contains an edge $vw$ with $v\in S_x$ and $w\in S_y$.  Therefore $G$ contains a $(k,d)$ $vw$-shortcut $P$.  By \clmref{s-subset}, $v\in V_x$ and $w\in V_y$.  If neither $x$ nor $y$ is a $T$-ancestor of the other, then $V_x$ and $V_y$ are disjoint.  By \yref{ancestor-edge} $N_x$ and $V_y$ are also disjoint.  By \yref{separator} $P$ contains an internal vertex $v'\in F_x$.  By \yref{ancestor-edge}, $v'\in Y_a$ for some strict $T$-ancestor $a$ of $x$.  But this implies that $v\in S_{a'}$ for some $T$-ancestor $a'$ of $a$, contradicting the assumption that $v\in S_x$.
\end{proof}



\begin{clm}\clmlabel{general-bag-size}
  The graph $I$ has a tree-decomposition whose largest bag has size at most $\binom{k+t}{t}$.
\end{clm}

\begin{proof}
  For the tree-decomposition $(C_x:x\in V(T))$ of $I$ we use the same tree $T$ used in the tree-decomposition of $H$. For each node $x$ of $T$, $C_x$ contains $x$ as well as every $T$-ancestor $a$ of $x$ such that $I$ contains an edge $ax'$ where $x$ is a $T$-ancestor of $x'$ (including the possibility that $x=x'$).
  ...
  Consider an arbitray node $x\in V(T)$ whose ancestors are $x_0,\ldots,x_{r-1},x_r=x$.  To avoid triple-subscripts in what follows, we abuse notation slightly by using $V_i$, $F_i$, and $N_i$,  as shorthands for $V_{x_i}$, $F_{x_i}$ and $N_{x_i}$, respectively.   

  If $x_\delta\in C_x$, it is because $x_\delta x'\in E(I)$ for some $T$-descendant $x'$ of $x$.  This implies $G^{k,d}$ contains an edge $vw$ with $v\in S_{x'}$ and $w\in S_{x_\delta}=S_\delta$.  This implies that $G$ contains a $(k,d)$ $vw$-shortcut $P_{vw}$.  Let $v'$ be the second-last vertex of $P_{vw}$ (so $v'w\in E(G)$).  
  
  Since $w\in S_{\delta}$, at least one of the following is true:
  \begin{enumerate}
    \item $w\in Y_\delta$; or 
    \item $G$ contains a $(k,d)$ $ww'$-longcut $P_{ww'}$ that has an internal vertex in $Y_{\delta}$.
  \end{enumerate}
  In the former case we define $P_{ww'}$ to be the path of length 0 that contains only $w=w'$.  Let $w''$ denote the first vertex of $P_{ww'}$ contained in $Y_{x_i}$.
  ...
  To see this, first observe that, for each $i\in\{1,\ldots,p\}$, $w_p\in V_{\delta}$ since, otherwise, an internal vertex of $P_{ww'}$ belong to $F_\delta$, which would imply (by \yref{ancestor-edge}) that $w\in S_{\delta'}$ for some $\delta' < \delta$, contradicting the assumption that $w\in S_\delta$.  Therefore $s_p\ge\delta$.  To see that $s_p>\delta-1$,
  ...
  Let $H^+$ denote the supergraph of $H$ with vertex set $V(T)$ and in which $xy\in E(H^+)$ if and only there exists some $z\in V(T)$ such that $x,y\in B_z$. Note that, by \tref{ancestor-edge}, $xy\in E(H^+)$ if and only if $x\in B_y$ or $y\in B_x$.
  
  Next we observe that $a_0,\ldots,a_p$ is a lazy walk\footnote{A \emph{lazy walk} in a graph $H$ is a walk in the non-simple graph $H'$ obtained by adding a self loop to each vertex of $H$.} in $H$.  Indeed, if $a_i\neq a_{i+1}$ for some $i\in\{0,\ldots,p-1\}$ then this is precisely because $w_i\in V_{a_i}$ but $w_{i+1}\not\in V_{a_i}$.  By definition, $w_i\in Y_{a_i'}$ for some $T$-descendant $a_i'$ of $a_i$.
  ...
  Finally, we are in a position to appeal to \cite[Lemma~24]{pilipczuk.siebertz:polynomial-arxiv} which states that the number of nodes in $\overrightarrow{H}^+$ that can be reached from any node $x$ by a directed path of length at most $k$ is at most $\binom{k+t-1}{t}$.
\end{proof}
...
\begin{thm}\thmlabel{modern-major-general}
  Let $\ell\ge 1$, $t\ge 2$, $d\ge 1$, and $k\ge 1$ be integers, let $G$ be a graph having a layered $H$-partition of layered width $\ell$ in which $H$ has treewidth at most $t-1$.  Then $G^{k,d}$ has a layered $I$-partition of layered width at most $O(t\ell k^2d^k)$ where the treewidth of $H$ is at most $\binom{k+t-1}{t}-1$.
\end{thm}

\begin{proof}
  The proof is almost immediate, except that the layering $\mathcal{L}$ of $G$ may not be a valid layering of $G^{k,d}$.  In particular, $G^{k,d}$ may contain an edge $vw$ with $v\in V_i$ and $w\in V_{i+j}$ for any $j\in\{0,\ldots,k\}$.  To resolve this, we use a new layering $\mathcal{L}':=\langle V_0',\ldots,V_h'\rangle$ in which $V_i'=\bigcup_{j=ki}^{ki+k-1} V_i$.  This increases the thickness given by \clmref{general-width} from $O(t\ell kd^k)$ to $O(t\ell k^2d^k)$.
\end{proof}

It is natural to ask if \thmref{modern-major-general} is tight.  We do not answer this question, but we do point out that, even when $t=1$ and $d=3$, an exponential dependence on $k$ is necessary.  If $G$ is a complete binary tree on $2^{k/2}$ leaves, then $G^{k,3}$ contains a clique $K$ of size  $s=2^{k/2+1}-1$.  If these vertices appear in $r$ parts of the $I$-partition, then the treewidth of $I$ is at least $r-1$.  On the other hand, these vertices must all appear on two consecutive layers, so the layered width of the partition is at least $s/(2r)$. Therefore any layered $I$ partition of $G^{k,d}$ of layered width $\ell'$ in which the treewidth of $I$ is at most $t'-1$ must satisfy
\[
