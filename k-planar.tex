...
\begin{lem}
	...
	\begin{compactenum}
		...
		\begin{compactenum}
			...
			\end{compactenum}
		\end{compactenum}
	\note{DW}{Should the vertices already grabbed by $P_1,P_2,P_3$ be mentioned in the setup (since the new paths $Q_1,Q_2,Q_3$ need to avoid the vertices already grabbed by $P_1,P_2,P_3$)?. Basically, I am concerned about what happens when distinct vertical paths in $T$ want to grab the same vertex.}
