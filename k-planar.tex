...
\referee{1}{2. page 1: ``A tree-decomposition $T$ consists of a tree $T$ and a collection
...
Here $\boxtimes$ is the strong product\footnote{The \emph{strong product} of graphs $A$ and $B$, denoted by $A\boxtimes B$, is the graph with vertex set $V(A)\times V(B)$, where distinct vertices $(v,x),(w,y)\in V(A)\times V(B)$ are adjacent if
	...
	%\end{compactitem}
}, and treewidth\footnote{A \emph{tree decomposition} $\mathcal{T}$ of a graph $G$ consists of a tree $T$ and a collection $\mathcal{T}=(B_x:x\in V(T))$ of subsets of $V(G)$ indexed by the nodes of $T$ such that
%:
%\begin{compactenum}[(i)]
%\item 
(i) for every $vw\in E(G)$, there exists some node $x\in V(T)$ with $v,w\in B_x$; and
%\item 
(ii) for every $v\in V(G)$, the induced subgraph $T[v] := T[\{x: v\in B_x\}]$ is connected.
%\end{compactenum}
...
%\cref{Tools} for formal definitions and see 
\cref{ProductExample} for an example. Loosely speaking, \cref{PlanarProduct} says that every planar graph is contained in the product of a tree-like graph and a path. This enables combinatorial results for graphs of bounded treewidth to be generalised for planar graphs (with different constants). 
...
This theorem has applications in diverse areas, including queue layouts  \citep{dujmovic.joret.ea:planar}, non-repetitive colouring  \citep{dujmovic.esperet.ea:planar}, $p$-centered colouring  \citep{debski.felsner.ea:improved}, and adjacency labelling \citep{DEJGMM}, which we explore in \cref{Applications}. For example, we prove that $k$-planar graphs have bounded non-repetitive chromatic number (for fixed $k$). Prior to the recent work of \citet{dujmovic.esperet.ea:planar}, it was even open whether planar graphs have bounded non-repetitive chromatic number.
...
\referee{2}{Page 3.  The sentence 'For each $vw \in E(G')$ there is a path $P$ in $G$
...
\begin{thm}[\citep{pilipczuk.siebertz:polynomial-soda}]
\label{ps}
...
\referee{2}{Page 6. Regarding (T1), technically a subtree of a rooted tree can be
...
\referee{2}{Page 6. Replace 'that satisfies (T2)' by 'that satisfies (T1)'.}
...
\referee{2}{Page 6. Replace 'parent in $T$' by 'parent in $T_0$'.}
...
\referee{1}{6. page 6, last paragraph of Lemma 2: This isn’t quite right since you
...
\referee{2}{Page 6. The term 'hierarchical decomposition' is not defined.}
...
\referee{2}{Page 7. Although it is clear for me, 'separation' has not been
...
\referee{1}{7. page 8, item (i): ``$S$ has small layered width with respect to the layering
...
\referee{1}{11. page 10, Claim 5: The name $Pww'$ is strange since in case 2. there is
no $w'$ defined.}
...
\referee{1}{3. Theorem 10: Unfortunately I have to suggest removing it. I don’t see
...
\begin{lem}[\citep{pilipczuk.siebertz:polynomial-arxiv}]
...
\referee{2}{Page 18. In point 1 in the proof of Lemma 9, I think '$\phi(w)$' should
be '$\alpha(w)$'.}
...
\referee{2}{Page 20.  Where does the bound $d(d-3)/2$ come from?  The naive bound I
compute is $d(d-1)/2$.}
...
\begin{thebibliography}{37}
	...
	\bibitem[Bonamy et~al.(2020)Bonamy, Gavoille, and
	...
	{\(\omega\)}.
	\newblock \emph{Int. J. Comput. Geometry Appl.}, 22\penalty0 (5):\penalty0
	439--470, 2012.
	\newblock \doi{10.1142/S0218195912500112}.
	...
	\bibitem[Debski et~al.(2020)Debski, Felsner, Micek, and
	Schr{\"{o}}der]{debski.felsner.ea:improved}
	...
	\newblock \doi{10.1137/1.9781611975994.136}.
	
	\bibitem[{Dvo{\v{r}}{\'a}k et~al.(2020)Dvo{\v{r}}{\'a}k, Huynh, Joret, and Liu}]{DHGLW}
	...
	and Wood]{dujmovic.joret.ea:planar}
	...
	\newblock \doi{10.1145/3477542}. 
	
	\bibitem[Dujmovi\'c et~al.(2020{\natexlab{b}})Dujmovi\'c, Esperet, Joret,
	...
	\newblock \doi{10.19086/aic.12100}.
	
	\bibitem[{Dujmovi{\'c} et~al.(2020)Dujmovi{\'c}, Esperet, Morin, Walczak, and Wood}]{DEMWW}
...
\newblock \doi{10.1017/S0963548321000213}. 
	...
	graphs.
	\newblock \emph{Adv. Math.}, 219\penalty0 (3):\penalty0 1070--1080, 2008.
	...
	\newblock \emph{Combinatorica}, 17\penalty0 (3):\penalty0 427--439, 1997.
	...
	\bibitem[Pilipczuk and Siebertz(2018)]{pilipczuk.siebertz:polynomial-arxiv}
