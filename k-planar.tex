...
\note{DW}{It would help to say that we consider a path to be an (unordered) subgraph. Would that be a problem anywhere else?}
...
Say $G$ is a 2-connected plane multigraph. Let $T$ be a spanning tree of $G$ rooted at a vertex $r$ on the outerface of $G$. A \defin{bipod} in $G$ (with respect to $T$) is any path $P_1SP_2$, where $S$ is a facial path in $G$, and each $P_i$ is a vertical path in $T$ whose lower endpoint is in $S$. Here it is possible that $S$ or $P_i$ is empty. So every subpath of a bipod is a bipod. A \defin{tripod} in $G$ (with respect to $T$) is either a bipod in $G$ or a subgraph of $G$ consisting of a facial cycle $F$ of $G$, plus three distinct vertical paths $P_1,P_2,P_3$ in $T$ where the lower endpoint of each $P_i$ is in $F$. \note{DW}{Need to think about degenerate cases.}. These definitions generalise definitions in \citep{DJMMUW20} where $G$ is assumed to be a plane triangulation, in which case $F$ is a 3-cycle. \note{DW}{We actually managed to omit the definition of `bipod' in \citep{DJMMUW20}!}

%In the original proof for planar triangulation, B_i is an edge.

\begin{lem}
Let $G_0$ be a 2-connected plane multigraph. Let $T$ be a spanning tree of $G_0$ rooted at some vertex $r$ on the boundary of the outerface of $G_0$. Let $k\in\{1,2,3\}$ and let $C = [P_1,\dots,P_k]$ be a cycle in $G_0$ with $r$ in the exterior of $C$, where $P_1, \dots, P_k$ are bipods in $G_0$. 
	\note{DW}{We need to assume that $|V(C)|\geq 3$ and ensure that when we apply induction $|V(Z_j)|\geq 3$. I think it is okay, but it needs an argument.}
	Let $G$ be the plane graph consisting of all the edges and vertices of $G_0$ in $C$ and the interior of $C$. Then $G$ has a partition $\PP$ into tripods with respect to $T$ such that:
\begin{compactitem}
...
\begin{proof}
\note{DW}{Can we assume $k=3$ by allowing $P_i=\emptyset$? That would clean up the proof. We could scrap $R_1,R_2,R_3$ and just use $P_1,P_2,P_3$.}
	
The proof is by induction on $|V(G)\setminus V(C)|$. If $V(G)\setminus V(C)=\emptyset$, then $G$ has just one face bounded by $C$ and the desired partition is $\PP=\{P_1,\ldots,P_k\}$.  The desired tree-decomposition of $G/\PP$ consists of a single bag $\{P_1,\ldots,P_k\}$ indexed by a 1-node tree. 

Now assume that $V(G)\setminus V(C)\neq\emptyset$. Partition the vertices of $C$ into paths $R_1$, $R_2$, and $R_3$ as follows:
\begin{compactenum}
	\item If $k=1$ then, since $C$ is a cycle, $P_1$ has at least three vertices, so $P_1=[v, P_1', w]$ for two distinct vertices $v$ and $w$. We set $R_1:=v$, $R_2:=P_1'$ and $R_3:=w$.
...
\note{DW}{For a cycle $C$ in a plane graph $G$, define $G\langle C\rangle$ to be the subgraph of $G$ consisting of $C$ and all the edges and vertices of $G$ in the interior of $C$?}
...
replacing $R_{i,j}^+\in\PP_j$ by $P_i$, 
replacing $R_{i+1,j}^-\in\PP_j$ by $P_{i+1}$, and
replacing $L_{i,j}\in\PP_j$ by $Y$. \note{DW}{What if $k\neq 3$? Then $P_2$ or $P_3$ might not be defined.}
