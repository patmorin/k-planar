...
\note{DW}{Use $\mathcal{S}$ for the given shortcut system and $\mathcal{P}$ for the partition?}

For each $x\in V(T)$, define $S_x := \{v\in V(G): a(v)= x\}$. Observe that $\mathcal{S}:=(S_x : x\in V(T))$ is a partition of $V(G)$.  Let $J:=G^\mathcal{P}/\mathcal{S}$ denote the resulting quotient graph. We consider $V(J)\subseteq V(T)$, where each $x\in V(J)$ is the vertex obtained by contracting $S_x$ in $G^{\mathcal{P}}$. (Nodes $x\in V(T)$ with $S_x=\emptyset$ do not contribute a vertex to $J$.)
...
compute is $d(d-1)/2$.}
\note{DW}{consecutive neighbours don't have to be counted. I will revise}
