...
% \documentclass{socg-lipics-v2019}
...
\title{The Structure of $k$-Planar Graphs}
\titlerunning{The Structure of $k$-Planar Graphs}

\author{Vida Dujmović}{School of Computer Science and Electrical Engineering, University of Ottawa}{vida.dujmovic@uottawa.ca}{}{Research  supported by NSERC.}
% , Ottawa, Canada (\texttt{vida.dujmovic@uottawa.ca}).
%                 Research  supported by NSERC and the Ontario Ministry of Research and Innovation.},\,\,
\author{Pat Morin}{School of Computer Science, Carleton University}{morin@scs.carleton.ca}{[0000-0003-0471-4118]}{Research supported by NSERC.}
% , Ottawa, Canada (\texttt{morin@scs.carleton.ca}).                 Research  supported by NSERC and the Ontario Ministry of Research and Innovation.},\,\, and
\author{David R. Wood}{School of Mathematics, Monash University}{david.wood@monash.edu}{}{Research supported by the Australian Research Council.}
% , Melbourne, Australia (\texttt{david.wood@monash.edu}). Research supported by the Australian Research Council.}
% }

\authorrunning{V. Dujmović, P. Morin, and D. R. Wood}

\keywords{$k$-planar graphs, layered partitions, queue-number, nonrepetitive colouring, centered colouring, $k$-nearest-neighbour graphs}

\Copyright{Vida Dujmović, Pat Morin, and David R. Wood}
...
\relatedversion{This paper has been submitted to Discrete \&
Computational Geometry. The current status, as of December 4th, 2019, is ``under review''.  A full version of this paper is available at
...
\item for every $v\in V(G)$, the number of paths in $\mathcal{P}$ with $v$ as an internal vertex is at most $d$.
\end{compactitem} 
...
This section defines concepts and results from the literature that are used in our work. 
...
They defined a class $\mathcal{G}$ of graphs to \emph{admit bounded layered partitions} if there exist $t,\ell\in\mathbb{N}$ such that every graph $G\in \mathcal{G}$ has an $H$-partition of layered width at most $\ell$ for some graph $H=H(G)$ of treewidth at most $t$. These definitions relate to strong products as follows. 
...
\begin{compactenum}[(a)]
\item an $H$-partition with layered width at most $\max\{2g,1\}$ where $H$ has treewidth at most $9$;
\item an $H$-partition with layered width at most $\max\{2g,3\}$ where $H$ has treewidth at most $4$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Shortcut Systems: Proof of \cref{modern-major-general}}
\label{Structure}
...
The \emph{$T$-depth} of a node $x\in V(T)$ is the length of the path, in $T$, from $x_0$ to $x$.  For each node $x\in V(T)$, define $T_x := T[\{y\in V(T):\mbox{$x$ is a $T$-ancestor of $y$}\}]$ to be the maximal subtree of $T$ rooted at $x$.  

We begin with a fairly standard trick of working with a normalised tree decomposition. 

\begin{lem}\lemlabel{nice-decomposition}
  For every graph $H$ of treewidth $t$, there is a rooted tree $T$ with $V(T)=V(H)$ and a $T$-decomposition $(B_x:x\in V(T))$ of $H$ of width at most $t$ that has following additional properties:  
...
We first point out more properties that are immediately implied by \cref{generalized-tripod}:
...
\begin{proof}[Proof of \cref{generalized-tripod}]
  Property~\yref{thickness} follows immediately from the fact that $V(T)=V(H)$ (a condition ensured by \lemref{nice-decomposition}) and the fact that $\mathcal{Y}$ has layered width at most $\ell$ with respect to $\mathcal{L}$.  Property \yref{separator} is immediate from the definitions of $F_x$ and $N_x$.  In particular, $(N_x,V(G)\setminus V_x)$ is a separation of $G$ with $F_x=N_x\cap(V(G)\setminus V_x)$. 
...
\begin{proof}
Apply \cref{generalized-tripod} to $G$ and let $\mathcal{L}$, $\mathcal{Y}$, $\mathcal{T}$, $T$, $Y_x$, $V_x$, $F_x$, and $N_x$ be defined as in \cref{generalized-tripod} (so that $(\mathcal{L},\mathcal{Y})$ is an $H$-decomposition of $G$ of layered width $\ell$ where $H$ has treewidth $t$.)

For a node $x\in V(T)$, we say that a shortcut $P\in\mathcal{P}$ \emph{crosses} $x$ if $Y_x$ contains an internal vertex of $P$, that is, $P=(v_0,\ldots,v_r)$ and $\{v_1,\ldots,v_{r-1}\}\cap Y_x\neq\emptyset$.  We say that a vertex $v\in V(G)$ \emph{participates} in $x$ if $v\in Y_x$, or $\mathcal{P}$ contains a shortcut $P$ with $v\in V(P)$ and $P$ crosses $x$. Let $X_v$ denote the set of nodes $x\in V(T)$ such that $v$ participates in $x$.

\begin{clm}\label{x-v-ancestor}
  For each $v\in V(G)$ there exists a (unique) node $a(v)\in X_v$ such that
  $a(v)$ is a $T$-ancestor of every node in $X_v$.
\end{clm}

\begin{proof}
  Let  $Z := \{v\} \cup \{\{v_1,\ldots,v_{r-1}\}:(v_0,\ldots,v_r)\in\mathcal{P}$, $v\in \{v_0,\ldots,v_r\}\}$
...
For each $x\in V(T)$, we define $S_x := \{v\in V(G): a(v)= x\}$.
Observe that $\mathcal{S}:=(S_x : x\in V(T))$ is a partition of $V(G)$.  Let $J:=G^\mathcal{P}/\mathcal{S}$ denote the resulting quotient graph and let $V(J)\subseteq V(T)$ in the obvious way, so that each $x\in V(J)$ is the vertex obtained by contracting $S_x$ in $G^{\mathcal{P}}$. (Nodes $x\in V(T)$ with $S_x=\emptyset$ do not contribute a vertex to $J$.)
...
Next we show that $\mathcal{S}$ has small layered width with respect to $\mathcal{L}=(V_1,\ldots,V_h)$:
...
\begin{proof}
  ...
  Thus, $a_0,\ldots,a_p$ is a lazy walk in $H^+$ of length $p\le k$ where the distance $s_i$ between $a_i$ and the root $x_0$ of $T$ is non-decreasing.  By removing repeated vertices this gives a path in the directed graph $\overrightarrow{H}^+$ obtained by directing each edge $xy\in E(H^+)$ from its $T$-descendant $x$ towards its $T$-ancestor $y$. Finally, we are in a position to appeal to \cite[Lemma~24]{pilipczuk.siebertz:polynomial-soda} which states that the number of nodes in $\overrightarrow{H}^+$ that can be reached from any node $x$ by a directed path of length at most $k$ is at most $\binom{k+t}{t}$.  Therefore, the number of $T$-ancestors $a$ of $x$ contained in $C_x$ is at most $\binom{k+t}{t}$, so $|C_x|\le \binom{k+t}{t}$. 
\end{proof}
...
%%%%%%%%%%%%%%%%%%%
\section{\boldmath $k$-Planar Graphs: Proof of \cref{k-planar}}
\label{sec-k-planar}
...
Redoing the calculation at the end of the proof of \cref{clm:general-width} then yields $ |S_x\cap Y_i| \le d\ell\left( 
       2 
...
\label{Applications}
Our main theorems have a number of applications including queue layouts, non-repetitive colourings and clustered colourings  of $k$-planar and $(g,k)$-planar graphs. We present here queue layout results and defer non-repetitive colourings and clustered colourings to \cref{applmore}.
...
\begin{lem}[\citep{dujmovic.joret.ea:planar}]
\label{qn}
If a graph $G$ has an $H$-partition of layered width $\ell$, then  $\qn(G)\le 3 \ell \, \qn(H) + \floor{\tfrac{3}{2}\ell}$.
\end{lem}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Graph Classes: Examples}
\label{Examples}
There are several examples of graph classes that can be obtained from a shortcut system applied to graphs of bounded Euler genus.  These include map graphs, string graphs, powers of bounded degree graphs and $k$-nearest-neighbour graphs.  We present here  $k$-nearest-neighbour graphs results and defer the others to \cref{exmore}.
...
\linenumbers
\section{\boldmath  $1$-Planar Graphs: proof of \cref{1-planar}}
\label{sec-1-planar}
...
\begin{cor}
  For every $(g,k)$-planar graph $G$,  $\pi(G)\le \max\{2g,3\}\cdot(6k^2+16k+10) 4^{\binom{k+5}{4}}$. 
...
Recall that the \emph{$k$-th power} of a graph $G$ is the graph $G^k$ with vertex set $V(G^k):=V(G)$, where $vw\in E(G^k)$ if and only if $\dist_G(v,w)\leq k$. If $G$ is planar with maximum degree $\Delta$, then $G^k$ is $2k(k+1)\Delta^{k}$-planar. Thus we can immediately conclude that bounded powers of planar graphs of bounded degree admit bounded layered partitions. However, the bounds we obtain are improved by the following lemma that constructs a shortcut system directly. 
...
$\dist_G(x,y)$  in $G$. Let $\PP:=\{P_{xy}: \dist_G(x,y)\in\{1,\dots,k\} \}$. Say $P_{xy}$ uses some vertex $v$ as an internal vertex. If $\dist_G(v,x)=i$ and $\dist_G(v,y)=j$, then $i,j\in\{1,\dots,k-1\}$ and $i+j\leq k$. The number of vertices at distance $i$ from $v$ is at most $\Delta^i$. Thus the number of paths in $\PP$ with $v$ as an internal vertex is at most 
...
\cref{qn,p-centered,p-centered-treewidth,PowerPartition,non-repetitive} immediately imply the following result:
