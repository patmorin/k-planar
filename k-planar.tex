...
\begin{thm}
\label{k-planar}
Every $k$-planar graph has a layered $H$-partition of layered width at most $15k^2 + 30k$ in which $H$ has treewidth at most $\treewidth$.
\end{thm}
...
The purpose of this section is to prove \cref{k-planar} and related structural results.  We begin with a more general abstraction called $(k,d)$-shortcut systems and prove a structural result, \cref{modern-major-general}, for graphs derived from $(k,d)$-shortcut systems.  The results on $k$-planar (\cref{k-planar}) and $(g,k)$-planar graphs (\cref{g-k-planar}) can then be obtained from \cref{modern-major-general} and existing results for planar and bounded-genus graphs.

\subsection{Shortcut Systems}

For a rooted tree $T$ and any node $x\in V(T)$, we define $T_x := T[\{y\in V(T):\mbox{$x$ is a $T$-ancestor of $y$}\}]$ to be the maximal subtree of $T$ rooted at $x$.  We begin with a fairly standard lemma about normalized tree decompositions:
...
\subsubsection{Generalized Tripod Partitions}

For the remainder of this section, $G$ is a graph and $(\mathcal{L}:=\langle V_1,\ldots,V_h\rangle,\mathcal{Y}:=(Y_x: x\in V(H)))$ is a layered partition of $G$ of layered width $\ell$ where the treewidth of the quotient graph $H=G/\mathcal{Y}$ is at most $t$.  In addition, $(B_x:x\in V(T))$ is a tree-decomposition of $H$ obtained by applying \lemref{nice-decomposition} to $H$.  The following lemma shows how to interpret these objects as a hierarchical decomposition of $G$.

\begin{clm}\clmlabel{generalized-tripod}
  ...
  \begin{compactenum}[(Y1)]
    ...
    \item\ylabel{separator} For every $x\in V(T)$, there is no edge $vw\in E(G)$ with $v\in V_x$ and $w\in V(G)\setminus N_x$. 
    \item\ylabel{ancestor-edge} For every $x\in V(T)$, $x$ has $t'\le t$ strict $T$-ancestors $a_1,\ldots,a_{t'}$ such that $F_x \subseteq \bigcup_{i=1}^{t'} Y_{a_i}$.
  \end{compactenum}
\end{clm}

Before proving \clmref{generalized-tripod} we point out more properties that are immediately implied by it:
...
\begin{proof}[Proof of \clmref{generalized-tripod}]
...
\subsubsection{Shortcut Systems}
...
As in the rest of this section, $(\mathcal{L},\mathcal{Y})$ denotes a layered $H$-partition of $G$; $T$ is a tree used in a tree-decomposition $(B_x:x\in V(T))$ of $H:=G/\mathcal{Y}$ satisfying the conditions of \lemref{nice-decomposition}; and, for each $x\in V(T)$, $Y_x$, $V_x$, $F_x$, and $N_x$ are defined as in \clmref{generalized-tripod}.
...
\begin{proof}
  ...
  \[  |S_x\cap V_i|\le d\ell\cdot\left(k+1 + k + \sum_{j=2}^k 2(k-j)\right) %= d(2k+1) + \sum_{i=1}^{k-2} i
...
Let $G_0$ be the plane graph obtained by adding a dummy vertex at each crossing in $G$. In this way, each edge $vw\in E(G)$ corresponds naturally to a path $P_{vw}$ of length at most $k+1$ in $G_0$.  Take $\mathcal{P}=\{P_{vw}: vw\in E(G)\}$ and observe that $\mathcal{P}$ is a $(k+1,2)$-shortcut for $G_0$ and that $G_0^{\mathcal{P}}\supseteq G$.  Specifically, $G_0^{\mathcal{P}}$ contains every edge and vertex of $G$ as well as the dummy vertices in $V(G_0)\setminus V(G)$ and their incident edges.  
...
\begin{enumerate}
  \item Contributes at most 2 vertices to $S_x\cap V_i$ for $j\in\{0,\ldots,\floor{(k+1)/2}$;
  \item Contributes at most 1 vertex to $S_x\cap V_j$ for $j\in\{\floor{(k+1)/2},\ldots,k+1\}$; 
  \item Contributes no vertices to $S_x\cap V_j$ for $j > k+1$.
\end{enumerate}
Redoing the calculation at the end of the proof of \cref{clm:general-width} then yields $|S_x\cap Y_i|\le d\ell(5k/2+5)=15k+30$.  With this change, the layered width of the partition given by \cref{modern-major-general} becomes $15k^2+30k$.  This establishes \cref{k-planar}.
...
\citet[Theorem~22]{dujmovic.joret.ea:planar} show that every genus-$g$ graph has a layered $H$-partition of layered width at most $\max\{2g,3\}$ such that $H$ has treewidth at most 4. Using this result and proceeding exactly as in \cref{sec-k-planar} establishes the following generalisation of \cref{k-planar}.

\begin{thm}
\label{g-k-planar}
Every $(g,k)$-planar graph has a layered $H$-partition of layered width at most 
$\max\{2g,3\}(5k^2 + 10k)$ in which $H$ has treewidth at most $\binom{k+5}{4}$.
\end{thm}

\note{PM}{I remove the $(g,1)$-specialization. It requires cutting surfaces.  For reference, the statement was:
\begin{thm}
\label{g-1-planar}
