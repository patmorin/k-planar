...
\begin{thm}\thmlabel{generalized-tripod}
  ...
  \begin{compactenum}[(Y1)]
    ...
    \item\ylabel{ancestor-edge} $x$ has $t'\le t$ $T$-ancestors $a_1,\ldots,a_{t'}$ such that $F_x \subseteq \bigcup_{i=1}^{t'} Y_{a_i}$.
  \end{compactenum}
\end{thm}

Before proving \thmref{generalized-tripod} we point out another properties that are immediately implied by it:
...
\begin{proof}
  ...
  Let $(Q_x: x\in V(I))$ denote the given $I$-partition of $G$.  For a node $x\in V(T)$, let $R_x:=\bigcup_{y\in C_x} Q_{y}$. For a node $x\in V(T)$, we say that a $(k,d)$ $vw$-shortcut $P$ in $G$ \emph{touches} $x$ if $V(P)\cap R_x\neq\emptyset$.  If $G$ contains a $vw$-shortcut that touches $x$, then we say that $v$ and $w$ \emph{participate} in $x$. 
