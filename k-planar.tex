...
\begin{proof}
  ...
  To begin, let $d_0=d$, and $d_1=\min\{i: V(F_d\cap Y_i)\neq\emptyset \}$.  Observe that $F_{d}=F_{d_0}$ is contained in the interior of $F_{d_1}$, i.e., $N_{d_0}\subseteq N_{d_1}-F_{d_1}$.   We claim that 
  \[  
     |\{\delta\in\{d_1,\ldots,d_0-1\}:  \mbox{there exists a $\delta$-walk} \}| \le \binom{k+1}{2} \enspace .
  \]
  To prove this claim, consider the graph $I$ with vertex set $V(I)=\{d_1,\ldots,d_0\}$ and $ij\in E(I)$ if and only $i=j$ or $|F_i\cap Y_j|\neq\emptyset$.  Any $\delta$-walk $v_0,\ldots,v_\ell$ with $\delta\in V(I)$ corresponds to a walk $\rho_0,\ldots,\rho_{\ell}$ in $I$, where $\rho_i=\max\{j : v_i\in N_{j}-F_j\}$.
  
  The graph $I$ has the following properties:
  \begin{enumerate}
    \item For each $i\in V(I)$, $i$ has at most 2 neighbours among $0,\ldots,i-1$.  If $i$ has two such neighbours, then one of them is $i-1$.
  ...
  \end{enumerate}
  These two properties imply that $I$ has a very simple structure.   We call $j\in\{d_1,\ldots,d_0\}$ a \emph{left endpoint} if there exists an edge $ij\in E(I)$ with $j < i - 1$.  Let $j_1>j_1>\cdots> j_p$ denote the sorted sequence of left endpoints and, for convenience, let $j_{0}=d$.  Then 
  \[
      E(I) \subseteq \{ (i,i+1): i\in\{d_1,\ldots,d_0-1\} \cup 
         \bigcup_{t=1}^p \{ ij_t : i\in\{ j_t,\ldots,j_{t-1} \}\} \enspace . 
  \]
  (See, \figref{i-graph}.)  Observe that shortest path from in $i$ from $d_0$ to $j_t$ has length at least $t$.  Let $X_t=\{j_t,\ldots,j_{t+1}\}$ and observe that the shortest path from $d_0$ to some value $i\in X_t$ that does not contain any $i' <i$ goes through $j_{t}$.  Therefore, $X_t$ contains at most $k+2-t$ values of $\delta$ such that there exists a $\delta$-walk.  Therefore, 
  \[  
