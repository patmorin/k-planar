...
For at least one application, the planarity of $H$ is critical in obtaining asymptotically tight bounds \cite{BDJM}. \cref{1_planar_product} is proven in the more general setting of $d$-framed multigraphs. This more general result implies a product structure theorem for $d$-map graphs in which $H$ is also planar.
...
\note{PM}{Move this section out of the introduction, into a ``preliminaries'' section?}
...
The following result is the main technical contribution of the paper.   Loosely speaking, it shows that if a graph $G$ has product structure, then so too does $G^\SS$ for every shortcut system $\SS$ of $G$.
...
\begin{table}
  \begin{center}
    \begin{tabular}{lccl}
      ...
      $(g,\delta)$-string & $\binom{\delta+4}{3}-1$ & $\max\{2g,3\}(\delta^4 + 4\delta^3 + 9\delta^2 + 10\delta+4)$ & \cref{StringPartition} \\
      ...
      $d$-map & $3^*$ & $d+ 3\floor{d/2} - 3$ & \cref{NewMapGraph} \\
      $(g,d)$-map & $\textcolor{red}{?}$ & $\textcolor{red}{?}$ & \textcolor{red}{\cref{NewMapGraph}} \\ \hline
      \\[.2ex]
      \multicolumn{4}{l}{$^*$ the bound also applies to the simple treewidth of $H$.}
    \end{tabular}
  \end{center}
  \caption{Summary of known product structure theorems.}
  \label{summary_table}
\end{table}
...
\begin{compactitem}
  ...
  \item \Cref{examples} uses \cref{Structure} to derive product structure theorems for string graphs, powers of bounded-degree graphs, and $k$-nearest neighbour graphs.
  \item \Cref{FramedSection} proves a product structure theorem for $d$-framed multigraphs in which the graph $H$ has treewidth $3$ and is planar.  Using this result, we derive product structure theorems for $1$-planar graphs and map graphs, again with a planar graph $H$.
  \item \Cref{Applications} quickly surveys applications of product structure and the consequences of the current work.
...
The purpose of this section is to prove \cref{ShortcutPartition} and its specialization to $k$-planar graphs \cref{kPlanarProduct}.


\subsection{Shortcut Systems}

\cref{ShortcutPartition} shows how, given a $(k,d)$-shortcut system $\SS$ of a graph $G$, an $H$-partition of $G$ can be used to obtain a $J$-partition of $G^{\SS}$ where the layered width  does not increase dramatically and the treewidth of $J$ is not much more than the treewidth of $H$.  For convenience, it will be helpful to assume that $\SS$ contains a length-1 $vw$-shortcut for every edge $vw\in E(G)$.  Since $G^\SS$ is defined to be a supergraph of $G$, this assumption has no effect on $G^{\SS}$ but eliminates special cases in some of our proofs.
...
In this section we give three examples of well-known graph families that can be expressed in terms of shortcut systems and which therefore have product structure theorems.
...
A similar proof to that of \cref{StringShortcut} shows that every $(g,\delta)$-string graph is $(g,2\delta^2)$-planar. Theorems~\ref{PlanarProduct}(b), \ref{ShortcutProduct}  and \ref{GenusProduct}(b):
...
\begin{thm}
\label{gdMapProduct}
For integers $g\geq 0$ and $d\geq 3$, let $\ell:= 14\max\{2g,3\}\,\binom{d}{2}$.
Then every $(g,d)$-string graph is contained in $H\boxtimes P \boxtimes K_{\ell}$ for some path $P$ and for some graph $H$ with treewidth $9$,
\end{thm}
...
\cref{d_framed_product_stucture} is used below to obtain product structure theorems for 1-planar multigraphs and for map graphs.  For simple 1-planar graphs it is essential that we allow frames with parallel edges.
...
For all integers $g\geq 0$ and $d\geq 3$, every $d$-map graph is a spanning subgraph of a $d$-framed graph.\todo{All I did here was remove references to $g$}
\end{lem}
...
For each face $F$ of $G_1$ that corresponds to a vertex $x$ of $G_0$, the size of $F$ equals the number of nations incident to $x$, which is at most $d$.
So every face of $G_1$ has size at most $d$.  Let $\hat{G}_1$ be the $d$-framed graph whose frame is $G_1$.  

To complete the proof, we now show that $\hat{G}_1$ contains $G$.
If $vw\in E(G)$ then the nation faces of $G_0$ corresponding to $v$ and $w$ have a vertex $x$ of $G_0$ in common.   If some face of $G_1$ corresponds to $x$ then $v$ and $w$ are on a common face of $G_1$ so $vw\in E(\hat{G}_1)$.  If no face of $G_1$ corresponds to $x$ then the only two nation faces of $G_0$ with $x$ on their boundary those corresponding to $v$ and $w$, in which case $vw\in E(G_1)\subseteq E(\hat{G}_1)$.  Therefore $G\subseteq\hat{G}_1$.
\end{proof}
...
\citet{ABGKP20} proved that every planar graph with treewidth at most $3$ has queue-number at most $5$. Thus the graph $H$ in \cref{1_planar_product} has queue-number at most $5$. \cref{qn,1_planar_product,NewMapGraph} then imply:
...
\begin{cor}
\label{dMapQueue}
Every $d$-map graph has queue-number at most $24 (d+3\floor{d/2}-3) \,  - \ceil{\tfrac{3}{2} (d+3\floor{d/2}-3)}$.
\end{cor}
...
\begin{cor}\label{nonrepetitive_cor}
  ...
  \begin{compactenum}
    ...
    % \item $(g,d)$-map: $g4^{O(d)}$
    % \item $k$-NN: $4^{O(k^6)}$
    \item $(g,d)$-map graph: \textcolor{red}{??}
    % \item
  \end{compactenum}
\end{cor}
