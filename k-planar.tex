...
We prove the following theorem that shows if a graph $G$ is a subgraph of $H\boxtimes P$ and $\PP$ is a $(k,d)$-shortcut system for $G$, then $G^{\mathcal{P}}$ is a subgraph of $J\boxtimes P$, where the treewidth of $J$ is bounded by a function of $k$, $d$, and the treewidth of $H$.
...
These theorems have applications in diverse areas, including queue layouts  \citep{dujmovic.joret.ea:planar}, non-repetitive colouring  \citep{dujmovic.esperet.ea:planar}, and $p$-centered colouring  \citep{debski.felsner.ea:improved},
...
Dujmović \etal\ define a class $\mathcal{G}$ of graphs to \emph{admit bounded layered partitions} if there exist $t,\ell\in\mathbb{N}$ such that every graph $G\in \mathcal{G}$ has an $H$-partition of layered width at most $\ell$ for some graph $H=H(G)$ of treewidth at most $t$.
...
\begin{compactenum}[(a)]
\item an $H$-partition of layered width $1$ for some planar graph $H$ of treewidth at most $8$, and \label{subthm:eight-one}
\item an $H$-partition of layered width $3$ for some planar graph $H$ of treewidth at most $3$.\label{subthm:three-tree}
\end{compactenum}
\end{thm}
...
At the core of their work is an elegant proof of the following result of  \citet{pilipczuk.siebertz:polynomial-soda} that was introduced in the context of centered colourings (see \cref{sec:centered-colourings}):
\begin{thm}[\citep{pilipczuk.siebertz:polynomial-soda}]\thmlabel{ps}
  Every planar triangulation $G$ has an $H$-partition $\mathcal{P}$
  such that $H$ has treewidth at most 8 and, for each $P\in\mathcal{P}$, $G[P]$ is a shortest path in $G$.
\end{thm}

Indeed, \cref{PlanarPartition}.\ref{subthm:eight-one} is a slight strengthening of \cref{thm:ps} in which $G$ has a distinguished vertex $r$ and, for each $P\in\mathcal{P}$ no two vertices of $P$ have the same distance to $r$.

\section{New Results}

Apex-minor-free graphs, addressed by \cref{ApexMinorFree}, are the largest minor-closed class that admit bounded layered partitions~\citep{dujmovic.joret.ea:planar}. However, the family of $k$-planar graphs is not minor-closed.  A graph $G'$ obtained from a $k$-planar graph $G$ by edge deletions and edge contractions may or may not be $k$-planar. \snote{DCG Reviewer \#1 writes: \emph{Isn't this trivial? A grid with all
diagonals will do.}  The other reviewer writes: \emph{Well,
such a construction is trivial: just take grids with both diagonals in
every cell.}  In response, I changed the following sentence from ``construct'' to ``observe''. Ok?} Indeed, \citet{dujmovic.eppstein.ea:structure} observe that even 1-planar graphs may contain arbitrarily large complete graph minors. Our results for $k$-planar graphs are the first of this type for a non-minor-closed class.

The following result is the main theorem of the paper. Loosely speaking, it shows that if a graph $G$ admits bounded layered partitions, then so too does $G^\PP$ for every $(k,d)$-shortcut system $\PP$ of $G$.
...
\begin{thm}
\label{k-planar}
Every $k$-planar graph has an $H$-partition of layered width at most $18k^2 + 48k+30$ in which $H$ has treewidth at most $\binom{k+4}{3}-1$. %$\treewidth$.
\end{thm}
...
% $(\mathcal{P},\mathcal{L})$ is an $H$-partition of
...
\begin{proof}
  That \tref{subtree-root} implies \tref{ancestor-edge} is a standard observation: If two subtrees intersect, then one contains the root of the other.  Thus, it suffices to construct width-$t$ tree decomposition that satisfies \tref{ancestor-edge}.
...
First we need to understand the relationship between sets $S_x\in\mathcal{S}$ and the related sets objects $Y_x$, $V_x$, $F_x$, and $N_x$.
...
\begin{proof}
  Recall that $S_x$ is defined by vertices that participate in $x$, and these are vertices that are either in $Y_x$ or in shortcuts that cross $Y_x$.  We say that a vertex $w\in Y_x$ \emph{contributes} a vertex $v\in S_x$ if $v$ participates in $x$.
  ...
  \[  |S_x\cap L_i|\le d\ell\cdot\left(k+1 + 2k + \sum_{j=2}^k 2(k-j)\right) %= d(2k+1) + \sum_{i=1}^{k-2} i
      = d\ell(k^2 +3) \enspace . \qedhere
  \]
\end{proof}


% From this point onward, the plan is to show that (i)~$\mathcal{S}$ has small layered width with respect to a $\mathcal{L}$ and that (ii)~$J$ has small treewidth.


\snote{Reviewer \#2 writes: \emph{Lemma 3: the property (T2) is sth peculiar; EVERY tree-decomposition of H has the property that if x and y are adjacent then the root of x and the root of y are comparable in the tree; I suggest to get rid of this one. The same story applies to Claim 4 where the authors give an elaborate proof of that fact again.} The reviewer is right about Lemma~3 (the other reviewer noticed it also) but not Claim~4.  In Claim~4, $T$ is the tree in a tree decomposition of $H$.  At this point we're setting up to show that $T$ can also be used to get a tree decomposition of $J$ without blowing up the width too much.

To address this, I've added the map breaking things down into Step~(i) and Step~(ii), and the following text.}
...
\begin{proof}
  ...
  \clmref{i-ancestor} ensures that, for every edge $ax'\in E(J)$, $a,x'\in C_{x'}$.  The connectivity of $T[a]:=T[\{x\in V(T):a\in C_x\}]$ follows from the fact that, for every node $x'\in T[a]$, every node $x$ on the path in $T$ from $x'$ to  $a$ is also a node of $T[a]$.  Therefore $(C_x:x\in V(T))$ is indeed a tree decomposition of $J$.  It remains to bound the size of each bag $C_x$.
...
\label{induction} The setup:
  \begin{compactenum}
    ...
    \item For each integer $j\ge 0$, let $L_j=\{v\in V(G):\dist_T(r,v)=j\}$.
    ...
    \begin{compactenum}
      ...
      \begin{compactenum}
        ...
        \item $|V(P_i)\cap L_j| \le 15$ for all integers $j\ge 0$.
      \end{compactenum}
    \end{compactenum}
  ...
  \begin{compactenum}
    \item $H$ is planar;
    \item for all integers $j\ge 0$, and all $x\in V(H)$, $|S_x\cap L_j|\le 15$;
...
Given a 1-plane graph $G$, add edges to make it edge-maximal so that it has an outer face $F=v_1v_2v_3$. Next, add a vertex $r$ adjacent to $v_1$, $v_2$, and $v_3$ to obtain an edge-maximal 1-plane graph $\overline{G}$ with one vertex $r$ of degree 3 on its outer face.
  ...
  Use the layering $\mathcal{L}=\langle L_0',L_1'\ldots\rangle$ where $L_i'=L_{2i}\cup L_{2i+1}$ for each integer $i\ge 0$. That this is a layering of $G$ follows from the fact that $\dist_{G'}(v,w)\le 2$ for every edge $vw\in E(G)$.  Since $|L_j\cap S_x|\le 15$ for every integer $j\ge 0$, $|L_i'\cap S_x|\le 30$ for every integer $i\ge 0$ and every $x\in V(H)$.
\end{proof}
...
% \begin{cor}
% \label{cor-1-planar}
% Let $G$ be a $1$-plane graph, let $r_0$ be an arbitrary vertex of $G$, let $G^+$ be a graph obtained by adding a dummy vertex at each crossing in $G$ and then arbitrarily adding non-crossing edges to obtain a triangulation.  Then $G$ has a layered $H$-partition $(\mathcal{L}=\langle L_0',L_1',\ldots\rangle,\mathcal{P})$ satisfying the conditions of \cref{1-planar} and such that $L_i' = \{v\in V(G): \floor{\dist_{G^+}(r_0,v)/2}=i\}$ for each integer $i\ge 0$.
% \end{cor}
...
%  which leads to the layering $\langle L_0,L_1,\ldots\rangle$ of $G^+$ where $L_i=\{v\in V(G^+): \dist_{T}(r,v) = i\}$.
%
%  The rest of proof remains unchanged until the point where we derive the layering $\mathcal{L}=\langle L_0',L_1',\ldots\rangle$ of $G$.  At this point, we set
%  \[
%    L_i' := \{v\in V(G): \floor{\dist_{T}(r,v)-2}=i\}
...
%   Since $|Z\cap L_i| \le 2g$, the same argument used in the proof of \cref{width-of-g-plus} shows that $|Z'\cap L_i|\le 2g\times 2\times (2k+1) = 4g(2k+1)$ for each integer $i\ge 0$.
%
...
%
%   Apply \cref{cor-k-planar} or (in the case $k=1$) \cref{cor-1-planar} to $G^*_1$ using $G^{*+}_1$ and the distinguished vertex $r^*$ to obtain a layered $H$-partition of $G^*_1$.  Since $G^*_1\supseteq G^*_0$ and $G^*_0$ is isomorphic to $G-Z'$, this gives a layered $H$-partition $(\mathcal{L}=\langle L_0',L_1',\ldots\rangle,\mathcal{P})$ of $G-Z'$ that satisfies the conditions of \cref{cor-k-planar} or \cref{cor-1-planar}, as appropriate.  In particular, for every integer $i\ge 0$,
%   \begin{align*}
%     L_i'& = \{v\in V(G-Z'):\floor{\dist_{G^*}(r^*,v)/(k+1)} = i\} \\
%         & = \{v\in V(G-Z'):\floor{\dist_{G^+}(r_0,v)/(k+1)} = i\} \enspace .
%   \end{align*}
%   It follows that $|L_i'\cap Z'|\le 4g(2k+1)(k+1) = 8gk^2+12gk+4g$.
%
...
%   \[
%     |L_i' \cap P| \le \begin{cases} 8gk^2+12gk+4g & \text{if $P=Z'$} \\
%                                     30 & \text{if $P\neq Z'$ and $k=1$} \\
...
\citet{pilipczuk.siebertz:polynomial-soda} and \citet{debski.felsner.ea:improved} use \cref{thm:ps} and \cref{PlanarPartition}.\ref{subthm:three-tree}, respectively, to show that, when $G$ is planar, or of bounded genus, $\chi_p(G)$ is polynomial in $p$.  We make use of the following lemma due to \citet[Lemma~15]{pilipczuk.siebertz:polynomial-arxiv}.
...
\begin{lem}[\citep{debski.felsner.ea:improved}]
\label{p-centered}
Every graph $G$ that has an $H$-partition of layered width at most $\ell$ has $\chi_p(G)\le \ell (p+1)\, \chi_p(H)$.
\end{lem}

\begin{proof}
   Let $(\mathcal{L}=\langle L_0,L_1,\ldots\rangle, \mathcal{P}=(B_x:x\in V(H))$ be an $H$-partition of $G$ having layered width at most $\ell$.
   Use a product colouring $\phi:V(G)\to \{1,\ldots,\ell\}\times\{0,\ldots,p\}\times\{1,\ldots,\chi_p(H)\}$.  For each integer $i\ge 0$ and each $x\in V(H)$, assign the colour $\phi(v):=(\alpha(v),\beta(v),\gamma(v))$ to each vertex $v\in L_i\cap B_x$ such that:
   \begin{compactenum}
     \item $\alpha(v)$ is unique among $\{\phi(w): w\in L_i\cap B_x\}$, which is possible
...
\[ \chi_p(G)\le (18k^2+48k+30)(p+1) \binom{p+ \binom{k+4}{3}-1}{ \binom{k+4}{3}-1}\enspace. \]
...
\[ \chi_p(G)\le 5 (p+3)(p+2)(p+1)^2\enspace. \]
%120 (p+1) \binom{p+3}{3}
%Every graph $G$ that has a layered $H$-partition of layered width at most $\ell$ in which $H$ has treewidth at most $t$ satisfies $\chi_p(G)\le \ell (p+1) (t+1)\binom{p+t}{t}$.
%Every 1-planar graph has a layered $H$-partition of layered width at most 30 where $H$ is planar and has treewidth at most 3.
\end{cor}
...
\[ \chi_p(G) \le \max\{2g,3\}\cdot(6k^2+16k+10) (p+1) \binom{p+\binom{k+5}{4}-1}{\binom{k+5}{4}-1}\enspace. \]
%O(gk^2)\times p^{O(k^3)}$.
%%Every graph $G$ that has a layered $H$-partition of layered width at most $\ell$ in which $H$ has treewidth at most $t$ satisfies $\chi_p(G)\le \ell (p+1) (t+1)\binom{p+t}{t}$.
...
% \snote{Deleted results on p-centered colouring since the bounded-degree results by \citet{debski.felsner.ea:improved} give better dependence on $\delta$.}
%
%
...
\cref{non-repetitive,StringPartition} also give bounds on the non-repetitive chromatic number and the $p$-centered colouring number of $(g,\delta)$-string graphs, but the bounds are weak, since such graphs $G$ have maximum degree at most $2\delta$, implying that $\pi(G) \leq (4+o(1))\delta^2$ and $\chi_p(G)\le p(64\delta)^2$ by results of \citet{DJKW16} and \citet{debski.felsner.ea:improved}, respectively.
...
\cref{PlanarPartition}(b), \cref{SurfacePartition}(b), \cref{modern-major-general,PowerShortcut} imply the following result, which with \cref{PartitionProduct} implies \cref{kPowerBasic} in the introduction.
...
\[\chi_p(G^k) \leq \max\{2g,3\}(2k^4+6k^2) \Delta^{k}  (p+1)
...
% Veit Wiechert.
% \newblock On the queue-number of graphs with bounded treewidth.
