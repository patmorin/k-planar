...
\begin{compactitem}
  ...
  \item \Cref{examples} uses \cref{ShortcutProduct,kPlanarProduct,gkPlanarProduct} to derive product structure theorems for bounded-degree string graphs, powers of bounded-degree graphs, map graphs, and $k$-nearest neighbour graphs.
...
For convenience, it will be helpful to assume that $\SS$ contains a length-1 $vw$-shortcut for every edge $vw\in E(G)$.  Since $G^\SS$ is defined to be a supergraph of $G$, this assumption has no effect on $G^{\SS}$ but eliminates special cases in some of our proofs.  For a given $H$-partition $\mathcal{H}:=\{B_x:x\in V(H)\}$ of a graph $G$, we are frequently interested in the unique vertex $x\in V(H)$ such that $B_x$ contains a particular vertex $v\in V(G)$.  We will write this concisely by writing that ``$P_x$ is the part in $\mathcal{H}$ that contains $v$.''
...
\begin{proof}
  ...
  It follows from \cref{short_path_in_h} that, for each $v\in V(H)$, $v$ participates in $x:=a(v)$.  Therefore, the contents of $S_x$ are limited to elements that are in $Y_x$ or vertices in shortcuts that cross $x$.  We say that a vertex $w\in Y_x$ \defin{contributes} a vertex $v\in S_x$ if $v=w$ or if some path in $\SS$ that contains $v$ has $w$ as an internal vertex.  We upper bound the number of vertices in $S_x\cap L_i$ by upper-bounding the number of vertices contributed to $S_x\cap L_i$ by each $w\in Y_x$.
...
For any integer $d\ge 3$, an embedded (multi)graph $G$ is \defin{$d$-framed} if it has a biconnected plane spanning (multi)subgraph $G_0$, whose facial cycles each have length at most $d$,  and such that the interior of each edge in $E(G)\setminus E(G_0)$ is contained in the interior of some face of $G_0$ \note{DW}{The word ``interior'' may be misinterpreted when refering to the outerface (see referee's comment). Shall we just write ``contained in some face''? Do we need to change this elsewhere?}. The embedded (multi)graph $G_0$ is called the \defin{frame} of $G$. This definition (for simple graphs) was introduced by \citet{BDGGMR}.
...
\cref{d_framed_product_stucture} is a consequence of the following technical lemma, which is an extension of the analogous result for plane graphs~\cite[Lemma~17]{DJMMUW20}. The following definition is a convenient way to establish the planarity of $H$ in \cref{d_framed_product_stucture}:  A $T$-decomposition of a graph $G$ is \defin{$t$-simple} if each of its bags has size at most $t+1$ and, for each $t$-element subset $\{v_1,\ldots,v_t\}\subseteq V(G)$, at most two bags contain $v_1,\ldots,v_t$.  To show the planarity of $H$ in \cref{d_framed_product_stucture} we use the fact that a graph has a 3-simple tree-decomposition if and only if it has treewidth at most 3 and is planar~\citep{kratochvil.vaner:note}. Finally, a \defin{BFS spanning forest} $T$ of $G$ rooted at a set $V_0\subseteq V(G)$ is a spanning forest of $G$ that contains a tree rooted at each vertex of $V_0$ with the property that for each $v\in V(G)$,
...
For readers already familiar with the proof in \cite{dujmovic.joret.ea:planar}, the main difference here is that a typical tripod in the proof of \cite[Lemma~17]{dujmovic.joret.ea:planar} consists of three upward paths in a BFS tree/forest whose lower endpoints belong to a single triangular face $\tau$.  In the current setting, a typical tripod consists of three upward paths belonging to a common face $\tau$ of $G_0$ plus all of the (at most $d$) vertices of $\tau$.
...
% \note{DW}{I think $G_0$ should be replaced by $G'$ throughout this proof.}
% \note{PM}{Agreed. Done.}
...
\note{DW}{I suggest we do not start a new paragraph here}
...
\note{PM}{Here we're using $f$ to denote a face, earlier we were using $F$.}
