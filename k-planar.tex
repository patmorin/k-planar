...
\begin{thm}\thmlabel{k-planar}
  Every $k$-planar graph has a layered $H$-partition of layered-width at most $24k^2 + 36k + 12$ in which $H$ has treewidth at most $k^3/6+k^2/2+k/3-1$.
\end{thm}
...
\begin{clm}\clmlabel{bag-size}
  The tree decomposition $\{B_x: x\in V(K)\}$ of $H$ contains no bag larger than $k^3/6 + k^2/2 + k/3$.
\end{clm}
...
\begin{proof}
  ...
  \begin{compactenum}[(i)]
    ...
    \item $\ell\le k+1$.
  \end{compactenum}
  We call any walk with properties (i)--(iv) a \emph{$\delta$-walk}. Therefore, to upper-bound $|B_x|$, it suffices to upper-bound the number of $\delta\in\{0,\ldots,d-1\}$ such that there exists a $\delta$-walk.  This is our goal for the remainder of this proof. 
  
  In order to avoid excessive levels of subscripts, we introduce the following abbreviated notation: For $i\in\{0,\ldots,d\}$, we use $F_i$, $N_i$, and $Y_i$, as shorthands for $F_{a_i}$, $N_{a_i}$, and $Y_{a_i}$, respectively.
  
  Let $d_0=d$, and $d_1=\min\{i: V(F_d\cap Y_i)\neq\emptyset \}$.  By \propref{ancestor-boundary}, $F_d$ is contained in the interior of  $F_{d_1}$, i.e., $N_{d_0}\subseteq N_{d_1}-F_{d_1}$.   Refer to \figref{d0d1}.  We will now prove that 
  \begin{equation}
     |\{\delta\in\{d_1,\ldots,d_0-1\}:  \mbox{there exists a $\delta$-walk} \}| \le \binom{k+1}{2} \enspace . \eqlabel{one-round}
  \end{equation}
  \begin{figure}
    \begin{center}
      \includegraphics{figs/d0d1}
    \end{center}
    \caption{$F_{d_1},\ldots,F_{d_0}$ and the resulting graph $I$.  For readability, the edges $\{id_1:i\in\{d_1-2,\ldots,d_0\}$ are not shown.}
    \figlabel{d0d1}
  \end{figure}
  ...
  \begin{compactenum}
    \item If $i=j$, then $ij$ is an edge of $I$ since $I$ contains the self-loop $ii$ for each $i\in V(I)$.
    
    \item If $j\neq i$ then assume, without loss of generality, that $j < i$ so that $a_i$ is a $K$-descendant of $a_j$.  Now $v_t\in V(N_i-F_i)$, but $v_{t+1}\not\in V(N_i-F_i)$ otherwise we would have $j\ge i$. Therefore, $v_{t+1}\in V(F_i)$.  By \propref{ancestor-boundary}, $v_{t+1}$ is in $Y_{j'}$ for some $j'< i$ and the edge $ij'\in E(I)$.  By \propref{children}, $v_{t+1}\not\in V(N_{j''}-F_{j''})$ for any $K$-descedant $j''>j'$ of $j'$.  Therefore $j'=\max\{j : v_{t+1}\in N_j-F_j\}$. Therefore, by definition $j'=\rho(v_{t+1})=j$, so the edge $ij=ij'\in E(I)$. 
  \end{compactenum}
  ...
  \end{compactenum}
  The edges in $E(I)$ can be partitioned into two classes:  A set $B(I)=\{ij\in E(I): |i-j|\le 1\}$ of \emph{boring edges} and a set $J(I)=\{ij\in E(I): |i-j|>1\}$ of \emph{jump edges}.  For a jump edge $ij$ with $i>j$, $j$ is called a \emph{landing} and $i$ is called a \emph{takeoff}.  The jump edges of $I$ satisfy the following properties:
  \begin{enumerate}[(J1)]
    \item \jlabel{single} Every $i\in\{d_1+1,\ldots,d_0\}$ is a takeoff for at most one jump edge.
    
    \item \jlabel{nested} For every jump edge $ij\in E(I)$ with takeoff $i$ and landing $j$, and for every $q\in\{j+2,\ldots,i\}$, if $q$ is a takeoff some jump edge, then this edge is $qj$.
  \end{enumerate}
  ...
  Now, consider the parent $a_{i-1}$ of $a_i$ in $K$.  By \propref{children} $F_i$ is a wedge of the Mercedes graph $M_{i-1}$.  Let $P_1,\ldots,P_3$ be the partition of $V(F_{i-1})$ guaranteed by \propref{ancestor-boundary}.  Without loss of generality, we can assume that $P_1=V(Y_{d_1}\cap F_{i-1})$.  Then, by \propref{tripod-feet}, $V(F_i)$ contains vertices from at most one of $P_2$ or $P_3$---say $P_2$---and suppose $P_2=V(F_{i-1}\cap Y_j)$ for some $j< i-1$.  This accounts for a single jump edge $ij$ for which $i$ is the takeoff.  All remaining vertices of $F_i$ are contained in $Y_{i-1}$, which only introduces the boring edge $(i,i-1)$.  Thus we have established that jump edges satisfy \jref{single}.
  
  To establish that jump edges satisfy \jref{nested} we can use the same argument used to establish that each $F_i$ contains a vertex of $Y_{d_1}$ to establish that each $F_q$ contains a vertex of $Y_j$ and therefore $qj\in E(I)$.  Since $q \ge j+2$, $qj$ is a jump edge and, by \propref{single}, it is the only jump edge with takeoff $q$.

  Let $j_0=d$ and let $j_1>j_2>\cdots> j_p$ denote the sequence of landings, in decreasing order.  Then \jref{single} and \jref{nested} imply that
  \[
      J(I) \subseteq J^+(I) :=
         \bigcup_{t=1}^p \{ ij_t : i\in\{ j_t,\ldots,j_{t-1}+1 \}\} \enspace . 
  \]
  and clearly $B(I)\subseteq B^+(I) := \{(i,i+1): i\in\{d_1+2,\ldots,d_0\}\}$.
  Any walk in $I$ is also a walk in the supergraph $I^+$ with $V(I^+)=V(I)$ and $E(I^+)=B^+(I)\cup J^+(I)$, so it we can focus our attention on walks $\rho_0,\ldots,\rho_{\ell}$ in $I^+$ that satisify properties (i')--(iv').  We will also call these $\delta$-walks.
  
  We are almost done proving \eqref{one-round}. Refer to \figref{one-round}.  For any landing $j_t$, $t>0$, $I^+[j_t,\ldots,d_0]$ contains the path $d_0,j_1,\ldots,j_t$ of length $t$.  A few moments consideration will also convince the reader that $d_0,j_1,\ldots,j_t$ is a shortest path, in $I[j_{t+1}-1,\ldots,d_0]$, from $d_0$ to $j_t$.  Therefore, by (iv') no $\delta$-walk can containd $j_t$ for any $t> k+1$.
  ...
  Next, observe that, in $I^+[j_{t+1}+1,\ldots,d_0]$ every path from $d_0$ to some vertex  $j_{t}+i$, $i\ge 0$ contains $j_t$.  Furthermore, the only edges of $I^+[j_{t+1}+1,\ldots,d_0]$ incident to vertices $j_t+i$, $i>0$ are boring edges.  Therefore, for any $i\ge 0$, the shortest path from $d_0$ to $j_t+i$ in $I^+[j_{t+1}+1,\ldots,d_0]$ has length $t+i$.  It follows that there are at most $k+2-t$ values $\delta\in\{j_{t+1},\ldots,j_t\}$ such that there exists a $\delta$-walk.
  
  Therefore, the total number of $\delta\in\{d_1+1,\ldots,d_0\}$ such that there exists a $\delta$-walk is at most
  \[
     \sum_{t=0}^{k+1} (k+2-t) = \sum_{t=1}^{k+2} t = \binom{k+1}{2}
  \]
  We are now very close to \eqref{one-round}, which asks for the number of $\delta$-walks for $\delta\in\{d_1,\ldots,d_0-1\}$.
  The preceding formula counts the $d_0$-walk, but does not count the $d_1$ walk.  These two effects cancel each other, thus establishing \eqref{one-round}.
  
  Refer to \figref{nested}.
  ...
  Observe that $i\ge s-1$ because $v_0,\ldots,v_i$ contains a vertex from each of $F_{d_0},\ldots,F_{d_{s-1}}$.  
  ...
  \begin{compactenum}[(i)]
    ...
    \item $\ell\le k+1$, so $\ell-i \le \ell-s \le \le k+1-s$.
  \end{compactenum}
  Exactly the same analysis used to establish \eqref{one-round} shows that
  \begin{equation}
     |\{\delta\in\{d_{s+1},\ldots,d_s-1\}:  \mbox{there exists a $\delta$-walk} \}| \le \binom{k+1-s}{2} \enspace . \eqlabel{one-round-s}
  \end{equation}

  We conclude that the maximum number of $\delta\in\{0,\ldots,d-1\}$ such that there exists a $\delta$-walk is at most
  \[
      \sum_{s=0}^{k+1} \binom{k+1-s}{2}
      = \sum_{s=0}^{k+1} \binom{s}{2}
      = \frac{k^3}{6} + \frac{k^2}{2} + \frac{k}{3} \ge |B_x| \enspace . \qedhere
  \]
\end{proof}
...
\begin{proof}[Proof of \thmref{k-planar}]
  ...
  By \clmref{bag-size}, $\mathcal{S}$ produces a graph $H=G/\mathcal{S}$ of treewidth at most $k^3/6+k^2/2+k/3-1$.  By \clmref{width-of-g-plus}, for each $x\in V(H)$ and each integer $i$, $|B_x\cap V_i|\le 24k+12$, so $|B_x\cap V_i'|\le (k+1)(24k + 12) = 24k^2+36k+12$. 
\end{proof}
...
\begin{proof}[Proof of \thmref{g-k-planar}]
  ...
  Since $|Z\cap V_i| \le 2g$, the same argument used in the proof of \lemlabel{width-of-g-plus} shows that $|Z'\cap V_i|\le 2g\times 4\times (2k+1) = 16gk+8g$ for all $i\in\{1,\ldots,h\}$.  
