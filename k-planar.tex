...
The remainder of the paper is organized as follows: In \secref{1-planar} we prove \thmref{1-planar}.  In \secref{k-planar} we prove \thmref{k-planar}.  
...
\begin{lem}\lemlabel{induction} The setup:
  ...
  \end{compactenum}
  Then $N$ has an $H$-partition $\{S_x : x\in V(H)\}$ such that
  \begin{compactenum}
    \item $H$ is planar;
