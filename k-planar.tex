...
\item $G / \PP$ has a tree-decomposition $\mathcal{T}$ of width at most 3 such that:
\begin{compactitem}
...
\begin{proof}
	\note{DW}{Can we always assume $k=3$ by allowing $A_i=\emptyset$? That would clean up the proof.}
...
\begin{compactitem}
	...
	\item the quotient $G_j / \PP_j$ has a tree-decomposition $\mathcal{T}_j=\{B_x:x\in V(D_j)\}$ of width at most 3 such that: 
	\begin{compactitem}
		\item some bag of $\mathcal{T}_j$ contains $R_{i,j}^+, R_{i+1,j}^-, S_{i,j}$, and 
...
Let $\PP$ be obtained from $\bigcup_{j\in\{1,\dots,m\}} \PP_j$ by replacing 
$R_{i,j}^+\in\PP_j$ by $P_i$, 
$R_{i+1,j}^-\in\PP_j$ by $P_i$, and
$S_{i,j}\in\PP_j$ by $Y$.
By construction, $\PP$ partitions $V(G)$ into tripods with respect to $T$ and $P_1,\ldots,P_k\in \PP$. 

We now construct the desired tree-decomposition of $G/\PP$. For each $j\in\{1,\dots,m\}$, apply the above replacements within the bags of $\mathcal{T}_j$, and let $u_j$ be the node of $D_j$ such that $R_{i,j}^+, R_{i+1,j}^-, S_{i,j} \in B_{u_j}$. Let $D$ be the tree obtained from the disjoint union of $D_1,\dots,D_m$ by adding one new node $u$ adjacent to $u_1,\dots,u_m$. Define $B_u:=\{P_1,P_2,P_3,Y\}$. We claim that $\{B_x:x\in V(D)\}$ is the desired tree-decomposition of $G/\PP$. By construction, every bag $B_x$ has size at most 4. 

First we show that for each part $X\in\PP$, the set $\{x\in V(D) : X\in B_x\}$ induces a subtree of $D$. If $X\not\in \{P_1,P_2,P_3,Y\}$ then $X$ is only in bags in some $D_j$, implying $\{x\in V(D) : X\in B_x\}$ induces a subtree of $D$. If $X\in \{P_1,P_2,P_3,Y\}$ then $X$ is in $B_u$, and if some part $X'$ of some $\PP_j$ was replaced by $X$ in the above construction, then $X'$ is in $B_{u_j}$ and $\{x\in V(D_j):X\in B_x\}$ induces a subtree of $D_j$, implying that  $\{x\in V(D) : X\in B_x\}$ induces a subtree of $D$ (since $uu_j\in E(D)$). 
...
Say $X'\in\PP'_j$ replaced by $X\in\PP_j$. Thus $X\subseteq V(G_j)\setminus V(Z_j)$. Since $Z_j$ separates $V(G_j)\setminus V(Z_j)$ and $V(G)\setminus V(G_j)$ and $XW\in E(G/\PP)$, it must be that $W\in \PP_j$ also, implying that $XW\in B_x$ for some node $x\in V(D_j)$. 
This concludes the proof that $(B_x:x\in V(D))$ is the desired tree-decomposition  of $G/\PP$. 

Every face $F$ of $G$ is a face of some $G_j$ or is $F$ \note{DW}{Avoid using $F$ for two different things}. In the latter case, if $B$ is the bag at $u$ then $V(F)\subseteq \bigcup_{X\in B} X$ since $V(F) \subseteq Y\in B$. In the first case, there is a bag $B$ of $\mathcal{T}_j$ such that $V(F)\subseteq \bigcup_{X\in B} X$. This property is maintained by the above replacements since 
$R_{i,j}^+ \subseteq P_i$, 
$R_{i+1,j}^- \subseteq P_i$, and
$S_{i,j}\subseteq Y$.
