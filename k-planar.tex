...
\begin{proof}
  ...
  \begin{compactenum}[(i)]
    ...
    \item $\ell\le k+1$.
  \end{compactenum}
  We call any walk with properties (i)--(iv) a \emph{$\delta$-walk}. Therefore, to upper-bound $|B_x|$, it suffices to upper-bound the number of $\delta\in\{0,\ldots,d-1\}$ such that there exists an $\delta$-walk.
  
  In the following, when $i$ is an integer, we will use $F_i$, $Y_i$, and so on as shorthands for $F_{a_i}$, $Y_{a_i}$, and so on, respectively.
  ...
  \begin{figure}
    ...
    \figlabel{d0d1}
  \end{figure}
  To prove this claim, consider the graph $I$ with vertex set $V(I)=\{d_1,\ldots,d_0\}$ and $ij\in E(I)$ if and only $i=j$ or $|F_i\cap Y_j|\neq\emptyset$.  Any $\delta$-walk $v_0,\ldots,v_\ell$ with $\delta\in V(I)$ corresponds to a walk $\rho_0,\ldots,\rho_{\ell}$ in $I$, where $\rho_i=\max\{j\in\{0,\ldots,d\} : v_i\in N_{j}-F_j\}$.
  
  The graph $I$ has the following properties:
  \begin{enumerate}
    \item For each $i\in V(I)$, $i$ has at most 2 neighbours among $d_1,\ldots,i-1$.  If $i$ has two such neighbours, then one of them is $i-1$.

    \item For each $ij\in E(I)$ with $j<i$, and each $i'\in\{j+2,\ldots,i\}$, the vertex $i'$ has at most 2 neighbours and these are contained in $\{i'-1,j\}$.
  \end{enumerate}
