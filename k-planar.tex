...
$n>0$, there is a `universal' graph $U_n$ with $n^{1+o(1)}$ 
...
\note{DW}{We can motivate \cref{1-planar} since treewidth 3 is best possible, and the planarity of $H$ is essential in the proof of \cref{1PlanarQueue}. Shall we argue this in the response? Or do we remove \cref{sec-1-planar}? We could keep the statement of \cref{1-planar} in the paper and refer to the arXiv paper for the proof. What do you think? All this depends on whether we can fix the errors below. }
...
\note{DW}{I agree with the referee.}
Thus any edge that is a spar of a kite $K$ is not part of a sail of any kite $K'$.
\note{DW}{This is wrong I think} 
Observe that any spar of $K$ is incident on exactly four \defin{kite faces} of $G$, each of which has three edges and two vertices of $G$ on its boundary.
...
Our proof of \cref{1-planar} follows quickly from the following technical lemma, which is an extension of the analagous result for plane graphs \cite{DJMMUW20}.
\begin{lem}
...
\begin{proof}
	...
	For each edge $vw$ of $G$, say that $vw$ \defin{passes through} each internal vertex on $P_{vw}$.
...
\citet{PS21} and \citet{DFMS21} use \cref{ps} and \cref{PlanarProduct}(b), respectively, to show that $\chi_p(G)$ is polynomial in $p$ when $G$ is planar or of bounded Euler genus.  We use the following lemma due to \citet[Lemma~15]{pilipczuk.siebertz:polynomial-arxiv}.

\note{DW}{Is \cref{p-centered-treewidth} in the journal version \citep{PS21}?}
...
\note{DW}{In the journal version \citet{DFMS21} (under the insistence of the referee :-) ) proved that $\chi_p(H_1\boxtimes H_2) \leq \chi_p(H_1) \cdot \chi(H_2^p)$ where $H_2^p$ is the $p$-th power of $H_2$. Apply this with $H_2:= P \boxtimes K_\ell$. Then $H_2^p= P^p\boxtimes K_\ell$ and $\chi(H_2^p)=(p+1)\ell$. So 
	$\chi_p(H\boxtimes P \boxtimes K_\ell) \leq \chi_p(H_1) \cdot (p+1)\ell$ }

\note{DW}{I suggest we replace the next paragraph by 	``\citet{DFMS21} proved that 
