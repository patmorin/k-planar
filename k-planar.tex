...
\note{PM}{The trick I used (using the thm counter everywhere) to get consecutive Theorem, Lemma, etc numbering has broken cleverref.  Everything is now references as a Theorem.  Find a solution that works.}
...
\subsection{\boldmath $d$-Framed and $1$-Planar Graphs}
\label{sec-1-planar}
...
We will actually prove this result in the more general setting of $d$-framed graphs \cite{BDGGMR}. For any integer $d\ge 3$, a (multi)graph $G$ is \defin{$d$-framed} if it admits a drawing having a biconnected plane skeleton (multi)graph $G_0$, whose faces have size at most $d$, and whose crossing edges are in the interior of these faces.  In \cref{1_planar_is_4_framed}, below, we show that every $1$-planar graph is a subgraph of a $4$-framed multigraph. This makes \cref{1-planar} an immediate consequence of the following theorem, whose proof is the subject of the rest of this section.

\begin{thm}\label{d_framed_product_stucture}
  Every $d$-framed graph is contained in $H\boxtimes P\boxtimes K_{\lfloor d/2\rfloor + d - 3}$ for some planar graph $H$ with treewidth at most 3 and for some path $P$.
\end{thm}


\subsection{\boldmath $d$-Framed Graphs}

To establish the planarity of $H$ in \cref{d_framed_product_stucture}, the following definition is helpful.
...
\begin{lem}
	...
	\begin{compactenum}
		\item Let $G$ be a $d$-framed graph and let $G'$ be the frame of $G$.
...
\begin{proof}
  ...
  \begin{compactenum}[(i)]
    ...
    \item The trivial tree decomposition of $H$ that places $x_1,\ldots,x_k$ is $3$-simple and, if $k=3$, has exactly one bag that contains $\{x_1,\ldots,x_3\}$.
  \end{compactenum}
  ...
  \begin{figure}
    ...
    \label{boring_figure}
  \end{figure}
  \todo[inline]{Update these figures with $d$-sided $\tau$.}
