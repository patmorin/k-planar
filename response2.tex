...
\begin{response}
  We have added the qualifier "bounded-degree" here, even though the $\delta$-string condition is actually stronger.  (A pair of strings may cross more than once, in which case each of these crossings counts against $\delta$.)   At this point in the introduction (not yet having defined a string graph), this is hopefully a good enough approximation.  The first paragraph of Section 3.1 gives the definition of $\delta$-string graph and makes note of the distinction between $\delta$-string graph and string graph of maximum degree at most $\delta$.  Hopefully anyone that wants to make use of Theorem~14 will read the definition of $\delta$-string graph.
\end{response}
